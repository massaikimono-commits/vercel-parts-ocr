-- ICB-SPEC v1.2: inspection customer-visit waiting-service rules.
-- Applied to shared Supabase as migration: add_waiting_service_visit_rules_v12
-- Captured from the live database immediately after application.
--
-- Meaning:
--   work_orders.is_waiting_service = true only when a customer arrives for an
--   inspection and waits on site until the inspection work is completed.
...
comment on column public.work_orders.is_waiting_service is
  'True only when a customer_visit inspection customer waits on site until the inspection work is finished. Never infer from notes, status, or stay_reason.';
