...
-- work_order_schedule_changes only allows CHECKIN/DELIVERY/COMPLETION/WORKER/REASON/OTHER.
-- Preserve reservation-change history under OTHER and keep the precise event in JSON.
...
begin
  ...
  where n.nspname='public' and p.proname='reschedule_schedule_entry_v2'
  order by p.oid desc
  limit 1;

  if v_oid is null then
    raise exception 'reschedule_schedule_entry_v2 not found';
  end if;
  ...
  if position(v_old_type in v_def) > 0 then
    ...
    raise exception 'expected schedule history change type not found';
  end if;

  if position('''eventType'',''schedule_entry_rescheduled''' in v_def) = 0 then
    if position(v_old_payload in v_def) = 0 then
      raise exception 'expected schedule history payload not found';
    end if;
