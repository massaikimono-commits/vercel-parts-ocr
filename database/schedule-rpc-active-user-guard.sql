-- ICB schedule RPC active-app-user hardening.
-- Shared Supabase migration: harden_schedule_rpcs_active_app_user
--
-- SECURITY DEFINER remains intentional for the existing atomic scheduling workflow.
-- Authorization rule:
--   1) trusted app-secret/server request => allowed
--   2) otherwise => caller must be an active ICB app user
--
-- This file is the later security overlay for the schedule RPC definitions recorded
-- in database/waiting-service-v12.sql. Do not restore the previous auth.uid()-only guard.
...
begin
  ...
  return public.create_schedule_registration_v2(
...
begin
  ...
  if v_waiting_service
    ...
    return jsonb_build_object(
      ...
      'hardErrors',jsonb_build_array('作業待ちは「点検・来社」の予定だけで使用できます。'),
      'overrideRequired',false
    );
  end if;
...
-- Keep direct browser/API access limited to signed-in app users; server/service paths remain available.
revoke all on function public.create_schedule_registration_batch_v1(date,jsonb,boolean) from public, anon;
grant execute on function public.create_schedule_registration_batch_v1(date,jsonb,boolean) to authenticated, service_role;

revoke all on function public.create_schedule_registration_batch_v1(uuid[],text,text,timestamp with time zone,timestamp with time zone,uuid,text,text,text,boolean,boolean,uuid,text,boolean,timestamp with time zone,timestamp with time zone,text,boolean) from public, anon;
grant execute on function public.create_schedule_registration_batch_v1(uuid[],text,text,timestamp with time zone,timestamp with time zone,uuid,text,text,text,boolean,boolean,uuid,text,boolean,timestamp with time zone,timestamp with time zone,text,boolean) to authenticated, service_role;

revoke all on function public.create_schedule_registration_v2(text,text,text,timestamp with time zone,timestamp with time zone,text,text,text,text,text,text,text,text,uuid,text,text,text,boolean,boolean,uuid,uuid,boolean,timestamp with time zone,timestamp with time zone,text,boolean) from public, anon;
grant execute on function public.create_schedule_registration_v2(text,text,text,timestamp with time zone,timestamp with time zone,text,text,text,text,text,text,text,text,uuid,text,text,text,boolean,boolean,uuid,uuid,boolean,timestamp with time zone,timestamp with time zone,text,boolean) to authenticated, service_role;

revoke all on function public.create_schedule_registration_v2(text,text,text,timestamp with time zone,timestamp with time zone,boolean,text,text,text,text,text,text,text,text,uuid,text,text,text,boolean,boolean,uuid,uuid,boolean,timestamp with time zone,timestamp with time zone,text,boolean) from public, anon;
grant execute on function public.create_schedule_registration_v2(text,text,text,timestamp with time zone,timestamp with time zone,boolean,text,text,text,text,text,text,text,text,uuid,text,text,text,boolean,boolean,uuid,uuid,boolean,timestamp with time zone,timestamp with time zone,text,boolean) to authenticated, service_role;

revoke all on function public.reschedule_schedule_entry_v2(uuid,timestamp with time zone,timestamp with time zone,boolean,text,text,date,text,boolean) from public, anon;
grant execute on function public.reschedule_schedule_entry_v2(uuid,timestamp with time zone,timestamp with time zone,boolean,text,text,date,text,boolean) to authenticated, service_role;

revoke all on function public.reschedule_schedule_entry_v2(uuid,timestamp with time zone,timestamp with time zone,text,text,date,text,boolean) from public, anon;
grant execute on function public.reschedule_schedule_entry_v2(uuid,timestamp with time zone,timestamp with time zone,text,text,date,text,boolean) to authenticated, service_role;
