...
function readActiveVehicle() {
  for (const storage of [sessionStorage, localStorage]) {
    try {
      const active = JSON.parse(storage.getItem(ACTIVE_KEY) || "null");
      if (active?.id) return active as { id: string };
    } catch {}
  }
  return null;
}
...
export default function CustomerVehiclesLayout({ children }: { children: ReactNode }) {
  ...
  function openLeaseMaintenance() {
    const active = readActiveVehicle();
    if (!active?.id) {
      alert("先に一覧から作業車両を選択してください。");
      return;
    }
    location.assign(`/customer-vehicles/lease-maintenance?vehicle=${encodeURIComponent(active.id)}`);
  }
  ...
  return (
    <>
      ...
      <nav className="activeVehicleDock" aria-label="選択車両の作業メニュー">
        ...
        <button onClick={openLeaseMaintenance}>リースメンテ契約</button>
      </nav>
      <style jsx global>{`
        ...
        @media(max-width:720px){.activeVehicleDock{left:8px;right:8px;bottom:8px;display:grid;grid-template-columns:1fr 1fr;gap:6px}.activeVehicleDock button{width:100%;padding:10px 8px}.page{padding-bottom:205px!important}}
