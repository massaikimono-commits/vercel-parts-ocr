...
type Vehicle={number:string;model:string;type:"EV"|"ガソリン"|"HV"|"その他";weight:string;registration:string;last4:string;chassis:string;firstRegistration:string;customerId:string};
type Customer={id:string;type:"individual"|"company";name:string;companyName:string;phone:string;email:string;postalCode:string;address:string;notes:string};
type Box={x:number;y:number;w:number;h:number};
type Template={widthMm:number;heightMm:number;fields:{name:Box;qty:Box;retail:Box;cost:Box}};
...
function parseOCR(text:string):Part[]{
  ...
  for(const raw of text.split(/\r?\n/)){
    ...
    if(n.length>=3) out.push({id:uid(),name:c[0],qty:n[0].replace(/[^\d.-]/g,""),retail:money(n[1]),cost:money(n[2]),source:line});
  }
  return out;
}
...
export default function Home(){
  ...
  const emptyVehicle:Vehicle={number:"",model:"",type:"EV",weight:"",registration:"",last4:"",chassis:"",firstRegistration:"",customerId:""};
  const [vehicle,setVehicle]=useState<Vehicle>(emptyVehicle);
  const [vehicles,setVehicles]=useState<Vehicle[]>([]);
  const emptyCustomer:Customer={id:"",type:"individual",name:"",companyName:"",phone:"",email:"",postalCode:"",address:"",notes:""};
  ...
  useEffect(()=>{try{
    const p=localStorage.getItem("parts-data"); if(p)setParts(JSON.parse(p));
    const t=localStorage.getItem("parts-template"); if(t)setTemplate(JSON.parse(t));
  }catch{}},[]);
  ...
  async function saveVehicle(){
    ...
    const normalized={...vehicle,last4:(vehicle.registration.replace(/\D/g,"").slice(-4)||vehicle.last4).slice(-4)};
    const payload={vehicle_number:normalized.number,model:normalized.model||null,fuel_type:normalized.type,vehicle_weight:normalized.weight?Number(normalized.weight):null,customer_id:normalized.customerId||null,chassis_number:normalized.chassis||null,first_registration:normalized.firstRegistration||null,registration_number_last4:normalized.last4||null,updated_at:new Date().toISOString()};
    const existing=(vehicle as any).id;
    const {data,error}=existing?await supabase.from("vehicles").update(payload).eq("id",existing).select().single():await supabase.from("vehicles").insert(payload).select().single();
    if(error){setMsg(`車両保存エラー: ${error.message}`);return}
  ...
  async function saveCustomer(){
    if(!customer.name.trim() && !customer.companyName.trim()){setMsg("顧客名または会社名を入力してください。");return}
    if(!session){setMsg("ログインしてください。");return}
    const payload={customer_type:customer.type,name:customer.name||customer.companyName,company_name:customer.companyName||null,phone:customer.phone||null,email:customer.email||null,postal_code:customer.postalCode||null,address:customer.address||null,notes:customer.notes||null,updated_at:new Date().toISOString()};
    const {data,error}=customer.id?await supabase.from("customers").update(payload).eq("id",customer.id).select().single():await supabase.from("customers").insert(payload).select().single();
    if(error){setMsg(`顧客保存エラー: ${error.message}`);return}
  ...
  function newCustomer(){setCustomer({...emptyCustomer,id:uid()});}
  function newVehicle(){setVehicle(emptyVehicle);}

  async function doOCR(file:File){
    setOcrBusy(true);setProgress(0);setMsg("");
    try{
      const {createWorker}=await import("tesseract.js");
      const worker=await createWorker("jpn+eng",1,{logger:m=>{
      ...
      setMsg(found.length?`${found.length}件を追加しました。必ず内容を確認してください。`:"OCR結果を確認してください。表形式の伝票は手修正が必要な場合があります。");
    }catch{setMsg("OCRに失敗しました。明るく真上から撮影して再試行してください。")}
    finally{setOcrBusy(false)}
  }

  function updatePart(id:string,key:keyof Part,val:string){setParts(p=>p.map(x=>x.id===id?{...x,[key]:val}:x))}
  function copyTSV(){
    const s=["部品名称\t個数\t定価\t仕入れ",...parts.map(p=>`${p.name}\t${p.qty}\t${p.retail}\t${p.cost}`)].join("\n");
    navigator.clipboard?.writeText(s);setMsg("Excel貼り付け用データをコピーしました。");
  }
  function csv(){
    const s=[["部品名称","個数","定価","仕入れ"],...parts.map(p=>[p.name,p.qty,p.retail,p.cost])]
      .map(r=>r.map(x=>`"${String(x).replaceAll('"','""')}"`).join(",")).join("\n");
    const a=document.createElement("a");a.href=URL.createObjectURL(new Blob(["\ufeff"+s],{type:"text/csv;charset=utf-8"}));a.download="parts.csv";a.click();
  }
  ...
  function importGuide(file:File){
    const r=new FileReader();r.onload=()=>setGuide(String(r.result||""));r.readAsDataURL(file);
  }

  const active=parts.find(p=>p.id===selected);

  if(authLoading) return <main><section className="card"><h1>読み込み中…</h1></section></main>;
  if(!session) return <main><header className="header"><div className="title">📷 部品伝票OCR・印刷</div><div className="desc">顧客・車両情報を安全に管理するためログインが必要です。</div></header><section className="card"><h1>ログイン</h1><input type="email" placeholder="メールアドレス" value={email} onChange={e=>setEmail(e.target.value)}/><input type="password" placeholder="パスワード（6文字以上）" value={password} onChange={e=>setPassword(e.target.value)}/><div className="actions"><button className="primary" onClick={async()=>{setAuthMsg("");const {error}=await supabase.auth.signInWithPassword({email,password});if(error)setAuthMsg(error.message);}}>ログイン</button><button onClick={async()=>{setAuthMsg("");const {error}=await supabase.auth.signUp({email,password});if(error)setAuthMsg(error.message);else setAuthMsg("登録しました。確認メールが届く設定の場合はメールを確認してください。");}}>新規登録</button></div>{authMsg&&<div className="notice">{authMsg}</div>}<p>※ 顧客・車両情報を扱うため、公開状態での無認証アクセスは行いません。</p></section></main>;

  return <main>
    <header className="header"><div className="title">📷 部品伝票OCR・印刷</div><div className="desc">部品名称・個数・定価・仕入れを読み取り、保存・Excel・指定用紙印刷まで</div></header>

    <nav className="tabs">
      {([["vehicle","①車体番号"],["customerVehicle","⑤顧客・車両管理"],["ocr","②伝票OCR"],["data","③データ"],["print","④印刷"],["settings","⑥印刷位置設定"]] as const).map(([id,label])=>
        <button key={id} className={tab===id?"tab active":"tab"} onClick={()=>setTab(id)}>{label}</button>)}
    </nav><div className="actions noPrint"><button onClick={async()=>{await supabase.auth.signOut();}}>ログアウト</button></div>
    {msg&&<div className="notice">{msg}</div>}

    {tab==="vehicle"&&<section className="card">
      <h1>車体番号</h1>
    ...
    {tab==="customerVehicle"&&<section className="card">
      <h1>⑤ 顧客・車両管理</h1>
      ...
      <div className="manage-grid">
        <div className="subcard">
          ...
          <div className="actions"><button className="primary" onClick={saveCustomer}>顧客を保存</button><button onClick={newCustomer}>新規</button></div>
        ...
        <div className="subcard">
          ...
          <div className="actions"><button className="primary" onClick={saveVehicle}>車両を保存</button><button onClick={newVehicle}>新規</button></div>
          ...
          <div className="list">{registrationFiltered.map(v=>{const c=customers.find(x=>x.id===v.customerId);return <button className="row" key={v.number} onClick={()=>setVehicle(v)}>{v.registration||v.number}　{v.model}　{c?.companyName||c?.name||"顧客未登録"}</button>})}</div>
        </div>
      </div>
    </section>}

    {tab==="ocr"&&<section className="card">
      <h1>部品伝票OCR</h1>
      <p>iPhoneで伝票を撮影してください。1回の撮影から複数部品を候補抽出し、部品名称・個数・定価・仕入れを編集できます。</p>
      ...
      <div className="actions"><button onClick={()=>setParts(p=>[...parseOCR(ocrText),...p])}>OCR結果から追加</button><button onClick={()=>setParts(p=>[...p,{id:uid(),name:"",qty:"1",retail:"",cost:""}])}>＋手入力</button></div>
      <h2>抽出データ</h2>
    ...
    {tab==="data"&&<section className="card">
      <h1>保存データ</h1><p>部品データはブラウザ内に保存されます。Excelへはタブ区切りでそのまま貼り付けできます。</p>
    ...
    {tab==="print"&&<section className="card">
      <div className="noPrint"><h1>④ 指定用紙へ印刷</h1>
      <p>添付された「部品出庫伝票」を基準に、背景は印刷せず、部品名称・個数・定価・仕入れだけを既存用紙の空欄へ重ね刷りします。</p>
      ...
      <div className="actions"><button className="primary" onClick={()=>window.print()}>🖨 印刷する</button><button onClick={()=>setTab("settings")}>印刷位置を調整</button></div></div>
      <div className="sheet">
        {parts.slice(0,printCount).map((p,i)=><div key={p.id}>
          {(["name","qty","retail","cost"] as const).map(f=>{
            const b=template.fields[f];
            return <div key={f} className={"overlay "+(f==="name"?"name":"")} style={{left:`${b.x}mm`,top:`${b.y+i*7}mm`,width:`${b.w}mm`,height:`${b.h}mm`}}>
              {f==="name"?p.name:f==="qty"?p.qty:f==="retail"?p.retail:p.cost}
            </div>
          })}
        </div>)}
      </div>
    ...
    {tab==="settings"&&<section className="card">
      <h1>⑤ 印刷位置設定</h1>
      <p>実物の用紙写真をガイドとして読み込み、4項目のX/Y/W/Hをmmで調整します。初回はコピー用紙でテストしてください。</p>
      ...
      <div className="positions">
        {(["name","qty","retail","cost"] as const).map(f=><div className="pos" key={f}><h3>{f==="name"?"部品名称":f==="qty"?"個数":f==="retail"?"定価":"仕入れ"}</h3>
      ...
      <div className="sheet preview">{guide?<img src={guide} alt="用紙ガイド"/>:<div className="placeholder">A4用紙ガイド<br/>用紙写真を読み込んでください</div>}
    ...
    <footer>部品伝票OCR・印刷 / Vercel対応</footer>
  </main>
}
