...
const DEBUG_ID = "certificate-kei-height-guard-v3-debug";
...
function showStatus(text) {
  ...
  if (!box) {
    ...
    box.innerHTML = '<summary style="font-weight:800">軽自動車 高さガード v3（確認用）</summary><div data-height-status style="margin-top:8px;font-weight:700"></div>';
    host.appendChild(box);
  }
  const node = box.querySelector("[data-height-status]");
  if (node) node.textContent = text;
}
function crop(source, region, targetWidth = 1600) {
...
export default function CertificateKeiHeightGuardV3() {
  useEffect(() => {
    ...
    const enforce = () => {
      ...
      if (Number.isFinite(now) && now > 200) {
        ...
        showStatus(`高さ ${now}cm は軽自動車として不正 → 採用せず高さセルだけ再確認`);
      }
    };
    ...
    const onChange = (event) => {
      ...
      running = false;
      lockedHeight = "";
      showStatus("v16完了後、高さ欄だけ1pass確認します");
    };
    ...
    const timer = window.setInterval(async () => {
      ...
      showStatus(`高さ ${currentHeight || "空欄"}cm → 幅・高さセルだけ1pass再読取中…`);
      let session = null;
      let canvas = null;
      try {
        session = await createDocumentRecognitionSession(file, { maxSide: 2150, cropPaper: true, minPaperConfidence: 0.38 });
        ...
        // 車両諸元1段目の右側だけ。後軸340kgの2段目を切り離し、幅147と高さ178を同時に見る。
        canvas = crop(source, [0.66, 0.505, 0.30, 0.052], 1650);
        ...
        const candidates = values.filter((n) => n >= 100 && n <= 200 && (!width || n !== width));
