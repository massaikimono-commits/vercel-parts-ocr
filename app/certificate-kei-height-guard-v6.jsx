...
const DEBUG_ID = "certificate-kei-height-guard-v6-debug";
...
function showStatus(text) {
  ...
  if (!box) {
    ...
    box.innerHTML = '<summary style="font-weight:800">軽自動車 高さガード v6（確認用）</summary><div data-height-status style="margin-top:8px;font-weight:700"></div>';
...
function process(ctx, width, height, mode) {
  ...
  for (let p = 0; p < image.data.length; p += 4) {
    ...
    const v = mode === "binary"
      ? (g < Math.max(120, Math.min(205, avg - 16)) ? 0 : 255)
      : Math.max(0, Math.min(255, Math.round((g - 128) * 1.65 + 150)));
...
function candidates(raw = "", widthValue = 0) {
  const text = norm(raw);
  const out = [];
  for (const token of text.match(/\d{3,4}/g) || []) {
...
export default function CertificateKeiHeightGuardV6() {
  useEffect(() => {
    ...
    const onChange = (event) => {
      ...
      showStatus("v16完了後、右端の高さセルを生画像・強調・二値の3表示で合計1pass確認します");
    };
    ...
    const timer = window.setInterval(async () => {
      ...
      // QRなど確実な値が来ていれば追加OCRしない。v16の弱い100-200値は再確認対象にする。
      ...
      const parts = [];
      try {
        session = await createDocumentRecognitionSession(file, { maxSide: 2700, cropPaper: true, minPaperConfidence: 0.38 });
        ...
        // 車両重量の数値行の一番右「高さ」値だけ。下段の後軸340を避ける。
        const box = [0.835, 0.455, 0.155, 0.048];
        parts.push(crop(source, box, "raw"));
        parts.push(crop(source, box, "contrast"));
        parts.push(crop(source, box, "binary"));
        ...
        if (!height) {
          ...
          pending = null;
          showStatus(`高さ未確定 / 1pass / OCR=${raw || "空"} / conf=${confidence.toFixed(1)} → 空欄維持`);
          return;
        }
        ...
        pending = null;
        showStatus(`高さ ${height}cm 採用・ロック ✓ / 1pass / 候補=${ranked.map(([n,c]) => `${n}x${c}`).join(",")} / OCR=${raw || "空"}`);
      } catch (error) {
