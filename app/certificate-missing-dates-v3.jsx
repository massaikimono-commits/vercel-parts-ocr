...
const DEBUG_ID = "certificate-missing-dates-v3-debug";
...
function showStatus(text) {
  ...
  if (!box) {
    ...
    box.innerHTML = '<summary style="font-weight:800">日付セル直読 v3（確認用）</summary><div data-date-status style="margin-top:8px;font-weight:700"></div>';
...
function chooseRegistration(values) {
  ...
  return filtered.find((v) => firstMonth && monthKey(v) === firstMonth) || filtered.sort((a, b) => ordinal(a) - ordinal(b))[0] || "";
}
function chooseRecord(values, registration) {
  const expiry = norm(window.__vehicleCertificateQrPriority?.inspectionExpiry || fieldInput("有効期間の満了する日")?.value || "");
  const filtered = values.filter((v) => plausible(v) && norm(v) !== expiry && norm(v) !== norm(registration || ""));
  return filtered.sort((a, b) => ordinal(b) - ordinal(a))[0] || "";
}
function preprocess(ctx, width, height, mode) {
  if (mode === "raw") return;
  ...
  for (let p = 0; p < image.data.length; p += 4) {
    ...
    const v = mode === "binary" ? (g < Math.max(125, Math.min(205, avg - 18)) ? 0 : 255) : Math.max(0, Math.min(255, Math.round((g - 128) * 1.7 + 150)));
...
function crop(source, [x, y, w, h], mode = "raw", targetWidth = 1500) {
  ...
  preprocess(ctx, c.width, c.height, mode);
  return c;
}
...
export default function CertificateMissingDatesV3() {
  useEffect(() => {
    ...
    const onChange = (event) => {
      ...
      showStatus("v16完了後、右上の記録年月日＋左上の交付年月日を合計1passで確認します");
    };
    ...
    const timer = window.setInterval(async () => {
      ...
      if (!needRecord && !needReg) { showStatus("日付2項目とも取得済み → OCR省略"); pending = null; return; }
      ...
      const parts = [];
      try {
        session = await createDocumentRecognitionSession(file, { maxSide: 2750, cropPaper: true, minPaperConfidence: 0.38 });
        ...
        // 実物レイアウトに合わせた2セル。各セルは生画像＋強調版を同一画像へ積み、recognizeは1回だけ。
        const recordBox = [0.57, 0.010, 0.39, 0.052];
        const regBox = [0.135, 0.112, 0.36, 0.052];
        parts.push(crop(source, recordBox, "raw"));
        parts.push(crop(source, recordBox, "contrast"));
        parts.push(crop(source, regBox, "raw"));
        parts.push(crop(source, regBox, "contrast"));
        ...
        showStatus(`1pass完了 / 記録=${patch.recordDate || "保留"} / 交付=${patch.registrationDate || "保留"} / OCR=${raw || "空"}`);
        pending = null;
      } catch (error) {
