...
const ITEMS = [
  ...
  { href: "/customer-vehicles", label: "車両検索" },
] as const;
...
export default function DesktopQuickNav() {
  ...
  return <>
    <nav className="desktopQuickNav" aria-label="PC共通ショートカット">
      {ITEMS.map((item) => { const active = isActive(pathname, item.href); return <Link key={item.href} href={item.href} prefetch={true} className={active ? "active" : ""} aria-current={active ? "page" : undefined}>{item.label}</Link>; })}
      <Link href={todayHref} prefetch={true} className={todayActive ? "active today" : "today"} aria-current={todayActive ? "page" : undefined}>今日</Link>
    </nav>
    ...
    <style jsx global>{`
      ...
      @media(min-width:981px){
        ...
        .desktopQuickNav a:hover{background:#f2f6fb;color:#244e82}.desktopQuickNav a.active{background:#e9f1fc;color:#174f91}.desktopQuickNav a.today{min-width:66px}.desktopQuickNav a:focus-visible{outline:3px solid #72a7ed;outline-offset:1px}.desktopQuickNavSpacer{display:block;height:38px}
      }
