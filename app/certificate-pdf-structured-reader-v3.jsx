...
function parseStructured(lines) {
  ...
  const topHeader = findLineIndex(lines, (t) => (t.includes("自動車登録番号又は車両番号") || t.includes("車両番号")) && (t.includes("初度登録年月") || t.includes("初度検査年月")) && t.includes("車体の形状"));
  ...
  const keiChassisHeader = findLineIndex(lines, (t) => t.includes("車台番号") && t.includes("乗車定員") && t.includes("最大積載量") && t.includes("車両重量") && t.includes("長さ") && t.includes("幅") && t.includes("高さ"));
  ...
  if (dimensionValue) {
    ...
    const m = text.match(/([A-Z]{1,6}[A-Z0-9]{0,8}-[A-Z0-9]{4,14})\s+(\d{2,4})\s*cm\s+(\d{2,4})\s*cm\s+(\d{2,4})\s*cm\s+(-|\d{1,5})\s*kg\s+(-|\d{1,5})\s*kg\s+(-|\d{1,5})\s*kg\s+(-|\d{1,5})\s*kg/i);
    if (m) {
  ...
  if (modelHeader >= 0) {
    ...
    if (next) {
      ...
      const fuel = ["軽油", "ガソリン", "揮発油", "電気", "LPG", "CNG", "水素"].find((v) => raw.includes(v));
      if (fuel) put("fuel", fuel);
      let displacement = null;
      if (fuel) {
        displacement = raw.match(new RegExp("(\\d+(?:\\.\\d+)?)\\s*(L|kW|KW)\\s+" + fuel, "i")) || raw.match(new RegExp(fuel + "\\s+(\\d+(?:\\.\\d+)?)\\s*(L|kW|KW)", "i"));
      }
      if (!displacement) displacement = raw.match(/(\d+(?:\.\d+)?)\s*(L|kW|KW)\b/i);
      if (displacement) put("displacementOrRatedOutput", String(displacement[1]) + " " + String(displacement[2]).toUpperCase());
    ...
    for (let i = sectionStart; i < sectionEnd; i += 1) {
      ...
      if (value && value !== "使用者に同じ" && !value.includes("所有者")) {
        ...
        for (let j = i + 1; j < Math.min(sectionEnd, i + 5); j += 1) {
          ...
          const address = addressText.replace(/^.*?住\s*所\s*/, "").replace(/\s*\[[0-9\s]+\]\s*$/, "").trim();
  ...
  if (expiryHeader >= 0) {
    for (let i = expiryHeader; i < Math.min(lines.length, expiryHeader + 7); i += 1) {
      ...
      if (value) { put("inspectionExpiry", value); break; }
    }
  }
  ...
  const strong = Boolean(patch.registrationNumber && patch.chassisNumber && patch.model && patch.vehicleName && patch.vehicleWeightKg && patch.grossVehicleWeightKg && patch.lengthCm && patch.widthCm && patch.heightCm && patch.engineModel && patch.fuel && found >= 22);
...
export async function parseVehicleCertificatePdfStructured(file) {
  ...
  try {
    ...
    return { patch: parsed.patch, strong: parsed.strong, confident: parsed.strong, found: parsed.found, totalCount: parsed.found, pageNumber: chosen.pageNumber, pageCount: pdf.numPages || 1, parser: "structured-v3" };
...
function vehicleCard() { return Array.from(document.querySelectorAll("section.card")).find((section) => section.querySelector("h2")?.textContent?.includes("車検証から読み取る")) || null; }
function showStatus(message, error = false) {
  const card = vehicleCard(); if (!card) return;
  let box = card.querySelector("[data-pdf-structured-v3-status]");
  if (!box) { box = document.createElement("div"); box.dataset.pdfStructuredV3Status = "1"; box.style.marginTop = "12px"; box.style.padding = "14px"; box.style.borderRadius = "14px"; box.style.border = "1px solid #a8ddbf"; box.style.fontWeight = "800"; card.querySelector(".actions")?.insertAdjacentElement("afterend", box); }
  box.textContent = message; box.style.background = error ? "#fff1f1" : "#effaf4"; box.style.borderColor = error ? "#efb7b7" : "#a8ddbf"; box.style.color = error ? "#922" : "#174c2e";
}
function showDebug(result, tokenCount) {
  const card = vehicleCard(); if (!card) return;
  let details = card.querySelector("[data-pdf-structured-v3-debug]");
  if (!details) { details = document.createElement("details"); details.dataset.pdfStructuredV3Debug = "1"; details.style.marginTop = "12px"; details.innerHTML = "<summary style='font-weight:800;cursor:pointer'>PDF構造読み取り v3 詳細（確認用）</summary><pre style='white-space:pre-wrap;word-break:break-word;max-height:520px;overflow:auto;background:#f8fafc;border-radius:10px;padding:10px;font-size:12px'></pre>"; card.appendChild(details); }
  const pre = details.querySelector("pre"); if (!pre) return;
  const rows = Object.entries(result.patch).map(([key, value]) => `${key}: ${value}`);
  pre.textContent = [`文字トークン: ${tokenCount}`, `構造取得: ${result.found}項目 / strong=${result.strong ? "YES" : "NO"}`, "", ...rows, "", "--- PDF構造行 ---", ...result.lines.map((line) => line.text)].join("\n");
}
function showPreview(canvas) {
  const card = vehicleCard(); if (!card) return;
  let img = card.querySelector("img[data-pdf-structured-v3-preview]");
  if (!img) { img = document.createElement("img"); img.dataset.pdfStructuredV3Preview = "1"; img.alt = "PDF車検証プレビュー"; img.style.display = "block"; img.style.width = "100%"; img.style.maxHeight = "560px"; img.style.objectFit = "contain"; img.style.borderRadius = "14px"; img.style.marginTop = "14px"; img.style.background = "#f4f6fa"; card.appendChild(img); }
  img.src = canvas.toDataURL("image/jpeg", 0.88);
}
function resetForm() { const button = Array.from(document.querySelectorAll("button")).find((candidate) => (candidate.textContent || "").includes("＋新規車両")); button?.click(); }
function applyPatch(patch) { window[PDF_PRIORITY_KEY] = patch; window[QR_PRIORITY_KEY] = null; window.dispatchEvent(new CustomEvent(AUTH_EVENT, { detail: patch })); }
function passToExisting(input) { input.dataset[PASS_KEY] = "1"; input.dispatchEvent(new Event("change", { bubbles: true })); }
...
export default function CertificatePdfStructuredReaderV3() {
  useLayoutEffect(() => {
    ...
    const onChange = async (event) => {
      ...
      const isPdf = file.type === "application/pdf" || /\.pdf$/i.test(file.name || ""); if (!isPdf) return;
      ...
      try {
        ...
        try {
          ...
          if (qrFound) { showStatus(`PDF ${chosen.pageNumber}ページ目: QRを検出。QR優先ルートへ引き継ぎます。`); passToExisting(input); return; }
          if (!parsed.strong) { showStatus(`PDF構造読み取り v3: ${parsed.found}項目。構造確信度不足のため既存OCRへフォールバックします。`); passToExisting(input); return; }
          resetForm(); await new Promise((resolve) => setTimeout(resolve, 0)); if (dead) return;
