...
// Prevent semantically different but visually similar labels from being confused.
// These are field names, never sample values or vehicle-specific knowledge.
const LABEL_RULES = {
  recordDate: { any: ["記録"] },
  registrationNumber: { any: ["車両", "登録"] },
  chassisNumber: { any: ["台"] },
  registrationDate: { any: ["交付", "登録"] },
  firstRegistration: { any: ["初度"] },
  inspectionExpiry: { any: ["満了"] },
  engineModel: { any: ["原動機"] },
...
function showDebug(state, lines = []) {
  ...
  if (!host) return;
  let box = document.getElementById("certificate-layout-recognition-v4-debug");
  if (!box) {
    box = document.createElement("details");
    box.id = "certificate-layout-recognition-v4-debug";
    box.style.marginTop = "12px";
    box.style.padding = "12px";
    box.style.border = "1px solid #cfd8e6";
    box.style.borderRadius = "12px";
    box.innerHTML = '<summary style="font-weight:800">共通意味＋セル境界OCR v4（確認用）</summary><pre style="white-space:pre-wrap;word-break:break-word;font-size:12px"></pre>';
...
function anchorAllowed(key, anchor) {
  if (!anchor) return false;
  const text = norm(anchor.matchedText || "");
...
function extractChassis(value = "") {
  ...
  // Japanese domestic chassis serials after the hyphen are numeric. Keeping the
  // suffix numeric also prevents a neighbouring model code from being concatenated.
...
function fuseEvidence(evidence, fuzzy = false) {
  ...
  // For fuzzy code groups choose a weighted medoid: an observed candidate that is
  // closest to all the other observations. This avoids inventing a string and is more
  // robust than simply trusting one high-confidence OCR read.
  const representative = [...winner].sort((a, b) => {
    const distanceScore = item => winner.reduce((sum, other) =>
      sum + (1 - codeSimilarity(item.value, other.value)) * other.weight, 0
    ) - item.weight * 0.15;
...
const SPECS = {
  ...
  registrationDate: { profile: { ...OCR_FIELD_PRESETS.date, minSimilarity: 0.50 }, extract: normalizeJapaneseDate, prefer: null, fuzzy: false, output: "registrationDate" },
  recordDate: { profile: { ...OCR_FIELD_PRESETS.date, minSimilarity: 0.50 }, extract: normalizeJapaneseDate, prefer: null, fuzzy: false, output: "recordDate" },
...
async function readField(session, worker, anchor, anchors, tokenSets, spec) {
  ...
  const fallback = relativeRegionFromAnchor(anchor, page.width, page.height, {
    ...
    gap: 0.003,
    width: 0.28,
    height: Math.max(0.03, (anchor.bbox.y1 - anchor.bbox.y0) / page.height * 1.45),
    padY: 0.004,
  });
  const regions = [...grid, { region: fallback, direction: "right", geometryScore: 0.42 }];
  const attempts = [];
  const seen = new Set();
  ...
  for (const item of regions) {
    ...
    if (!fused) continue;
    const directionBonus = spec.prefer && item.direction === spec.prefer ? 0.28 : 0;
...
export default function CertificateLayoutRecognitionV4() {
  useEffect(() => {
      ...
      try {
        ...
        try {
          ...
          if (Object.keys(patch).length) {
            window.__vehicleCertificateLayoutV4Patch = patch;
            for (let i = 0; i < 8; i += 1) {
              if (stopped || myToken !== token) return;
          ...
          showDebug("意味＋セル境界OCR v4 完了", lines);
        } finally {
          await worker.terminate().catch(() => {});
        }
      } catch (error) {
        showDebug("意味＋セル境界OCR v4 エラー", [String(error?.message || error)]);
      }
    };
