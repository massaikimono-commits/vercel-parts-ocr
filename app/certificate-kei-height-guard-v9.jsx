...
const DEBUG_ID = "certificate-kei-height-guard-v9-debug";
...
function showStatus(text) {
  ...
  if (!box) {
    ...
    box.id = DEBUG_ID;
    box.style.marginTop = "10px"; box.style.padding = "10px"; box.style.border = "1px solid #69a985"; box.style.borderRadius = "12px"; box.style.background = "#f0fdf4";
    box.innerHTML = '<summary style="font-weight:800">軽自動車 高さガード v9（確認用）</summary><div data-height-status style="margin-top:8px;font-weight:700"></div>';
...
function process(ctx, width, height, mode) {
  ...
  for (let p = 0; p < image.data.length; p += 4) {
    ...
    const v = mode === "binary" ? (g < Math.max(118, Math.min(210, avg - 10)) ? 0 : 255) : Math.max(0, Math.min(255, Math.round((g - 128) * 1.9 + 162)));
    image.data[p] = image.data[p + 1] = image.data[p + 2] = v; image.data[p + 3] = 255;
  }
  ctx.putImageData(image, 0, 0);
}
function crop(source, [x, y, w, h], mode = "raw", targetWidth = 2200) {
...
function composite(canvases) {
  const width = Math.max(...canvases.map((c) => c.width)), gap = 28;
  ...
  let y = 0; for (const c of canvases) { ctx.drawImage(c, 0, y); y += c.height + gap; }
  return out;
}
function repairDigitLike(s = "") {
  return String(s).toUpperCase().replace(/[OQD]/g, "0").replace(/[IL|!]/g, "1").replace(/Z/g, "2").replace(/S/g, "5").replace(/G/g, "6").replace(/B/g, "8").replace(/\D/g, "");
}
function collectCandidates(raw, widthValue) {
  const counts = new Map();
  const text = norm(raw).toUpperCase();
  const chunks = text.match(/(?:[0-9OQDIL|!SZGB][\s.,:;\-]*){3,5}/g) || [];
  for (const chunk of chunks) {
    const d = repairDigitLike(chunk);
    if (d.length === 3) {
      const n = Number(d);
      if (n >= 100 && n <= 220 && n !== widthValue) counts.set(n, (counts.get(n) || 0) + 2);
    } else if (d.length === 4 && d.endsWith("0")) {
      const n = Number(d.slice(0, 3));
      if (n >= 100 && n <= 220 && n !== widthValue) counts.set(n, (counts.get(n) || 0) + 1);
    }
  }
  const ranked = [...counts.entries()].sort((a,b) => b[1] - a[1]);
  if (!ranked.length) return { value: 0, ranked };
  const [top, score] = ranked[0], second = ranked[1]?.[1] || 0;
  return { value: score >= 2 && score > second ? top : 0, ranked };
}
...
export default function CertificateKeiHeightGuardV9() {
  useEffect(() => {
    ...
    const enforce = () => {
      if (!isKei()) return;
      const input = fieldInput("高さ cm"); if (!input) return;
      const n = Number(norm(input.value || ""));
      if (locked) { if (input.value !== locked) setReactInputValue(input, locked); return; }
      if (Number.isFinite(n) && (n < 100 || n > 220)) setReactInputValue(input, "");
    };
    ...
    const onChange = (event) => {
      ...
      showStatus("v16完了後、幅・高さ行を英数字許容で1pass確認します");
    };
    const timer = window.setInterval(async () => {
      ...
      if (existing >= 100 && existing <= 220) { locked = String(existing); pending = null; showStatus(`既存高さ ${locked}cm を維持 → OCR省略`); return; }
      const qrHeight = Number(norm(window.__vehicleCertificateQrPriority?.heightCm || ""));
      if (qrHeight >= 100 && qrHeight <= 220) { locked = String(qrHeight); setReactInputValue(fieldInput("高さ cm"), locked); pending = null; showStatus(`QR高さ ${locked}cm を採用 → OCR省略`); return; }

      running = true;
      const file = pending, mine = generation, begun = performance.now();
      let session = null, combo = null; const parts = [];
      try {
        ...
        parts.push(crop(source, [0.56, 0.438, 0.435, 0.115], "raw", 2300));
        parts.push(crop(source, [0.56, 0.438, 0.435, 0.115], "contrast", 2300));
        parts.push(crop(source, [0.66, 0.452, 0.335, 0.090], "binary", 2100));
        ...
        if (!picked.value) {
          setReactInputValue(fieldInput("高さ cm"), ""); pending = null;
        ...
        locked = String(picked.value); setReactInputValue(fieldInput("高さ cm"), locked);
        window.dispatchEvent(new CustomEvent(AUTH_EVENT, { detail: { heightCm: locked } })); pending = null;
        showStatus(`高さ ${locked}cm 採用・ロック ✓ / ${Math.round(performance.now()-begun)}ms / 1pass / 候補=${picked.ranked.map(([n,s])=>`${n}:${s}`).join(",")} / OCR=${raw || "空"}`);
      } catch (error) {
        if (!stopped && mine === generation) showStatus(`高さガードエラー: ${error?.message || error}`);
      } finally {
        for (const c of parts) { c.width = 1; c.height = 1; } if (combo) { combo.width = 1; combo.height = 1; }
        releaseSession(session); running = false;
      }
    }, 260);
    document.addEventListener("change", onChange, true); window.addEventListener(AUTH_EVENT, onAuthoritative);
    return () => { stopped = true; generation += 1; window.clearInterval(timer); document.removeEventListener("change", onChange, true); window.removeEventListener(AUTH_EVENT, onAuthoritative); };
  }, []);
  return null;
}
