...
// Semantic rules are form vocabulary only. They do not contain any sample values.
const LABEL_RULES = {
  recordDate: { all: ["記録"] },
  registrationNumber: { all: ["番号"], any: ["車両", "登録"] },
  chassisNumber: { all: ["台", "番号"] },
...
function showDebug(state, lines = []) {
  ...
  if (!host) return;
  let box = document.getElementById("certificate-layout-recognition-v5-debug");
  if (!box) {
    box = document.createElement("details");
    box.id = "certificate-layout-recognition-v5-debug";
    box.style.marginTop = "12px";
    box.style.padding = "12px";
    box.style.border = "1px solid #cfd8e6";
    box.style.borderRadius = "12px";
    box.innerHTML = '<summary style="font-weight:800">共通意味＋列境界OCR v5（確認用）</summary><pre style="white-space:pre-wrap;word-break:break-word;font-size:12px"></pre>';
...
function codeIsAmbiguous(result) {
  ...
  // If visually-confusable OCR strings remain close in evidence, hold rather than
  // inventing O/0, S/5, I/1, etc. A clear repeated winner is still allowed.
...
const SPECS = {
  ...
  chassisNumber: { profile: { ...OCR_FIELD_PRESETS.code, minSimilarity: 0.54 }, extract: extractChassis, prefer: "right", fuzzy: true, output: "chassisNumber", allowAmbiguousCode: true },
  registrationDate: { profile: { ...OCR_FIELD_PRESETS.date, minSimilarity: 0.50 }, extract: normalizeJapaneseDate, prefer: "right", fuzzy: false, output: "registrationDate" },
  recordDate: { profile: { ...OCR_FIELD_PRESETS.date, minSimilarity: 0.50 }, extract: normalizeJapaneseDate, prefer: "right", fuzzy: false, output: "recordDate" },
  engineModel: { profile: { ...OCR_FIELD_PRESETS.code, minSimilarity: 0.52 }, extract: extractEngine, prefer: "right", fuzzy: true, output: "engineModel", allowAmbiguousCode: false },
...
async function evaluateRegions(session, worker, regions, tokenSets, spec) {
  const page = session.prepared.normalized;
  const attempts = [];
  const seen = new Set();

  for (const item of regions) {
    const key = [item.region.x, item.region.y, item.region.width, item.region.height]
    ...
    if (!fused) continue;
    const directionBonus = spec.prefer && item.direction === spec.prefer ? 0.30 : 0;
...
async function readField(session, worker, anchor, anchors, tokenSets, spec) {
  ...
  const preferred = spec.prefer ? grid.filter(item => item.direction === spec.prefer) : grid;

  // A preferred semantic geometry is authoritative when it yields a value. This keeps
  // a numeric value below 長さ from losing to a different number found to its right.
  if (preferred.length) {
    const primary = await evaluateRegions(session, worker, preferred, tokenSets, spec);
    if (primary.length) return primary[0];
  }

  const secondary = spec.prefer ? grid.filter(item => item.direction !== spec.prefer) : [];
  const fallbackDirection = spec.prefer || "right";
  const fallback = relativeRegionFromAnchor(anchor, page.width, page.height, {
    direction: fallbackDirection,
    gap: 0.003,
    width: fallbackDirection === "below" ? 0.16 : 0.28,
    height: fallbackDirection === "below"
      ? 0.045
      : Math.max(0.03, (anchor.bbox.y1 - anchor.bbox.y0) / page.height * 1.45),
    padY: 0.004,
  });
  const backup = [
    ...secondary,
    { region: fallback, direction: fallbackDirection, geometryScore: 0.38 },
  ];
  const attempts = await evaluateRegions(session, worker, backup, tokenSets, spec);
  return attempts[0] || null;
}

export default function CertificateLayoutRecognitionV5() {
  useEffect(() => {
    ...
    const onChange = async event => {
      ...
      try {
        ...
        try {
          ...
          showDebug("意味候補を先に絞り込み＋列境界検出完了", lines);
          ...
          for (const [key, spec] of Object.entries(SPECS)) {
            ...
            if (spec.fuzzy && !spec.allowAmbiguousCode && codeIsAmbiguous(result)) {
              lines.push(`${key}: ${result.value} 付近で O/0・S/5 等の候補割れ → 保留`);
              continue;
            }
          ...
          if (Object.keys(patch).length) {
            window.__vehicleCertificateLayoutV5Patch = patch;
            for (let i = 0; i < 8; i += 1) {
              if (stopped || myToken !== token) return;
          ...
          showDebug("意味＋列境界OCR v5 完了", lines);
        } finally {
          await worker.terminate().catch(() => {});
        }
      } catch (error) {
        showDebug("意味＋列境界OCR v5 エラー", [String(error?.message || error)]);
      }
    };
