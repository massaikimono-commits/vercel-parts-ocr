...
export default function VehicleWorkflowFast({ newRegistrationMode = false }: { newRegistrationMode?: boolean }){
  ...
  function select(v:Vehicle){setVehicle(v);sessionStorage.setItem(ACTIVE_KEY,JSON.stringify(v));setMessage(`${display(v)} を作業車両に選択しました。`);}
  ...
  return <>
    ...
    <section className="card"><h2>車検証読み取り情報</h2><div className="grid">{FIELDS.map(([k,l])=><label key={k}><span>{l}</span><input value={cert[k]||""} onChange={e=>update(k,e.target.value)}/></label>)}</div><div className="axles"><b>軸重</b><span>前前 {cert.frontFrontAxleWeightKg||"未読"} kg</span><span>前後 {cert.frontRearAxleWeightKg||"未読"} kg</span><span>後前 {cert.rearFrontAxleWeightKg||"未読"} kg</span><span>後後 {cert.rearRearAxleWeightKg||"未読"} kg</span></div><div className="actions">{!newRegistrationMode&&<button onClick={()=>setVehicle({...EMPTY,certificate:emptyCert()})}>＋新規車両</button>}<button onClick={save}>車両を保存</button></div><button className="primary wide" onClick={startOCR}>この車両で伝票OCRへ →</button></section>
