...
export default function Page(){
  ...
  const [authoritative,setAuthoritative]=useState<AuthoritativePatch>({});
  ...
  const effectiveVehicle=useMemo(()=>mergeVehicle(vehicle,authoritative),[vehicle,authoritative]);
  ...
  useEffect(()=>{
    const apply=(event:Event)=>{
      ...
      setAuthoritative(prev=>({...prev,...patch}));
  ...
  function select(v:Vehicle){setAuthoritative({});setVehicle(v);localStorage.setItem(ACTIVE_KEY,JSON.stringify(v));setMessage(`${display(v)} を作業車両に選択しました。`);}
  function update(k:string,val:string){
    setAuthoritative(prev=>({...prev,[k]:val}));
    setVehicle(prev=>{
  ...
  async function read(file:File){
    ...
    setAuthoritative({});setDocBusy(true);setProgress(1);setDebug("");setMessage("車検証の各欄を読み取り中です…");if(preview)URL.revokeObjectURL(preview);setPreview(URL.createObjectURL(file));let worker:any=null;
    try{
      ...
      const finalAuthority:AuthoritativePatch={};
      for(const key of ["registrationDate","firstRegistration","inspectionExpiry","bodyShape","model","frontFrontAxleWeightKg","rearRearAxleWeightKg","fuel"]){
        const value=c[key]; if(value)finalAuthority[key]=value;
      }
      setAuthoritative(finalAuthority);
  ...
  async function save(){const active=effectiveVehicle;if(!active.number.trim()&&!active.chassis.trim()){setMessage("車台番号または登録番号を確認してください。");return;}const c=active.certificate,p:any={vehicle_number:active.number.trim()||active.chassis.trim()||active.registration.trim(),registration_number:active.registration.trim()||null,registration_number_last4:(active.last4||active.registration.match(/(\d{4})(?!.*\d)/)?.[1]||"").slice(-4)||null,chassis_number:active.chassis.trim()||null,model:active.model.trim()||null,fuel_type:active.type,vehicle_weight:active.weight?Number(active.weight):null,curb_weight_kg:toInt(c.vehicleWeightKg),gross_vehicle_weight_kg:toInt(c.grossVehicleWeightKg),seating_capacity:toInt(c.seatingCapacity),engine_model:c.engineModel||null,usage_category:c.purpose||null,body_type:c.bodyShape||null,inspection_certificate_number:c.documentNumber||null,user_name_snapshot:c.userName||null,first_registration:jpMonth(active.firstRegistration)||active.firstRegistration||null,inspection_expiry_date:dateIso(c.inspectionExpiry),certificate_fields:c,front_front_axle_weight_kg:toInt(c.frontFrontAxleWeightKg),front_rear_axle_weight_kg:toInt(c.frontRearAxleWeightKg),rear_front_axle_weight_kg:toInt(c.rearFrontAxleWeightKg),rear_rear_axle_weight_kg:toInt(c.rearRearAxleWeightKg),customer_id:active.customerId||null,updated_at:new Date().toISOString()};const q=active.id?await supabase.from("vehicles").update(p).eq("id",active.id).select().single():await supabase.from("vehicles").insert(p).select().single();if(q.error){setMessage(`車両保存エラー: ${q.error.message}`);return;}const v={...active,id:q.data.id,number:p.vehicle_number,last4:p.registration_number_last4||"",firstRegistration:p.first_registration||""};setVehicle(v);setAuthoritative({});setVehicles(old=>[v,...old.filter(x=>x.id!==v.id)]);localStorage.setItem(ACTIVE_KEY,JSON.stringify(v));setMessage("車検証情報を保存し、作業車両に設定しました。");}
  function startOCR(){const active=effectiveVehicle;if(!active.number&&!active.chassis&&!active.registration){setMessage("先に車両を選択または保存してください。");return;}localStorage.setItem(ACTIVE_KEY,JSON.stringify(active));try{const a=JSON.parse(localStorage.getItem("parts-data")||"[]");localStorage.setItem(BEFORE_KEY,JSON.stringify(Array.isArray(a)?a.map((x:any)=>x.id).filter(Boolean):[]));}catch{localStorage.setItem(BEFORE_KEY,"[]");}location.assign("/ocr/auto");}

  const shown=effectiveVehicle;
  return <main className="page"><div className="top"><button onClick={()=>location.assign("/")}>← メインへ</button><strong>icb</strong></div>
    <section className="card"><h1>作業車両を選択</h1><p>車体番号・ナンバー下4桁・車台番号・型式で検索できます。</p><div className="notice">{busy?"車両一覧を読み込み中…":message}</div><input className="search" placeholder="車体番号 / 下4桁 / 車台番号 / 型式" value={search} onChange={e=>setSearch(e.target.value)}/><div className="list">{filtered.map(v=><button key={v.id||v.number} className={`row ${shown.id===v.id&&v.id?"active":""}`} onClick={()=>select(v)}><b>{v.registration||v.number}</b><span>{v.model||"型式未入力"}　下4桁 {v.last4||"----"}</span><small>{v.chassis||v.number}</small></button>)}</div></section>
    ...
    <section className="card"><h2>基本情報</h2><div className="grid"><label>登録番号<input value={shown.registration} onChange={e=>update("registrationNumber",e.target.value)}/></label><label>ナンバー下4桁<input value={shown.last4} onChange={e=>setVehicle(prev=>({...prev,last4:digits(e.target.value).slice(-4)}))}/></label><label>車台番号<input value={shown.chassis} onChange={e=>update("chassisNumber",e.target.value)}/></label><label>型式<input value={shown.model} onChange={e=>update("model",e.target.value)}/></label><label>燃料<select value={shown.type} onChange={e=>setVehicle(prev=>({...prev,type:e.target.value as FuelType}))}><option>EV</option><option>ガソリン</option><option>HV</option><option>ディーゼル</option><option>その他</option></select></label><label>車両重量 kg<input value={shown.weight} onChange={e=>update("vehicleWeightKg",e.target.value)}/></label><label>初度登録（和暦）<input value={shown.firstRegistration} placeholder="令和2年4月" onChange={e=>update("firstRegistration",e.target.value)}/></label></div></section>
    <section className="card"><h2>車検証読み取り情報（備考欄より上）</h2><p>読み取り後にここで全部確認・修正できます。</p><div className="grid">{FIELDS.map(([k,l])=><label key={k}><span>{l}</span><input value={shown.certificate[k]||""} onChange={e=>update(k,e.target.value)}/></label>)}</div><div className="axles"><b>軸重</b><span>前前 {shown.certificate.frontFrontAxleWeightKg||"未読"} kg</span><span>前後 {shown.certificate.frontRearAxleWeightKg||"未読"} kg</span><span>後前 {shown.certificate.rearFrontAxleWeightKg||"未読"} kg</span><span>後後 {shown.certificate.rearRearAxleWeightKg||"未読"} kg</span></div><div className="actions"><button onClick={()=>{setAuthoritative({});setVehicle({...EMPTY,certificate:emptyCert()});}}>＋新規車両</button><button onClick={save}>車両を保存</button></div><button className="primary wide" onClick={startOCR}>この車両で伝票OCRへ →</button></section>
