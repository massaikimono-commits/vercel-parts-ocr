...
import { DAILY_REPORT_TEMPLATE, dailyReportRowSlots } from "../daily-report-template";
...
type WorkOrder = { id: string; reason: string; worker_name: string | null; expected_completion_date: string | null; work_completed: boolean };
...
export default function DailyReportPrintPage() {
  ...
  async function load() {
    ...
    try {
      const [scheduleRes, vehicleRes, customerRes, workRes, settingRes] = await Promise.all([
        ...
        supabase.from("work_orders").select("id,reason,worker_name,expected_completion_date,work_completed"),
  ...
  return (
    <main>
      ...
      {!backgroundUrl && <div className="warning noPrint">既存の日報原本は公開GitHubへ置かない方針のため、背景ファイルが安全な保存先に設定されるまでは最終印刷を無効にしています。23行への配置結果は下で確認できます。</div>}
