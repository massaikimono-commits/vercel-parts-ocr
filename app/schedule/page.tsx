...
export default function SchedulePage() {
  ...
  const dayItemMap = useMemo(
    () => new Map(enriched.map((item) => [item.entry.id, item])),
    [enriched],
  );
  const dailyBoardModel = useMemo(
    () => buildDailyReportPreviewModel(
      morning.map(({ entry }) => entry),
      afternoon.map(({ entry }) => entry),
    ),
    [morning, afternoon],
  );
  ...
  function renderCard(item: typeof enriched[number]) {
    ...
    return (
      <article
        ...
        onClick={(event) => openScheduleDetailFromCard(event, entry)}
      >
        <div className="itemTop">
  ...
  function renderDailyBoardCell(item: typeof enriched[number] | undefined, side: "delivery" | "inbound") {
    if (!item) return <div className="dailySlotEmpty" />;
    const { entry, work, vehicle, customer } = item;
    const visitLabel = entry.entry_type === "customer_visit" || entry.entry_type === "onsite_repair"
      ? ENTRY_LABEL[entry.entry_type]
      : "";
    ...
    return (
      <article
        key={entry.id}
        data-work-id={work?.id || undefined}
        className={`dailySlotCard ${reasonClassName(work)} ${work?.is_urgent ? "urgentItem" : ""} ${work && focusWorkId === work.id ? "focusedWork" : ""}`}
        onClick={(event) => openScheduleDetailFromCard(event, entry)}
      >
        <div className="dailyCellCustomer">
          <b>{customerLabel(customer)}</b>
          <span className="dailyWorkState">{workStateControl(work)}</span>
        </div>
        <div className="dailyCellVehicle">
          <b>{last4Label(vehicle)}</b>
          <small>{work?.reason || "入庫要因未設定"}</small>
        </div>
        <div className="dailyCellTime">
          {visitLabel && <small>{visitLabel}</small>}
          <b>{timeLabel(entry)}</b>
        </div>
        {side === "inbound" && (
          <div className="dailyCellDue">
            {due.date && <b>{due.date}</b>}
            {due.time && <small>{due.time}</small>}
          </div>
        )}
      </article>
    );
  ...
  function dailyReportStyleBoard() {
    return (
      <section className="card dailyBoardSection">
        <div className="sectionTitle dailyBoardTitle">
          <div>
            <h2>1日の予定</h2>
            <small>手書き日報と同じ配置：午前は上詰め／午後は下詰め</small>
          </div>
          <span>{morning.length + afternoon.length}件</span>
        </div>
        <div className="dailyBoardHeader">
          <b>納車</b>
          <b>入庫</b>
        </div>
        <div className="dailyBoardRows">
          {dailyBoardModel.rows.map((row) => {
            const deliveryItem = row.delivery ? dayItemMap.get(row.delivery.id) : undefined;
            const inboundItem = row.inbound ? dayItemMap.get(row.inbound.id) : undefined;
            return (
              <div className="dailyBoardRow" key={row.slotIndex}>
                <div className="dailyBoardCell deliveryCell">{renderDailyBoardCell(deliveryItem, "delivery")}</div>
                <div className="dailyBoardCell inboundCell">{renderDailyBoardCell(inboundItem, "inbound")}</div>
              </div>
            );
          })}
        </div>
        {(dailyBoardModel.overflow.deliveries.length > 0 || dailyBoardModel.overflow.inbound.length > 0) && (
          <div className="overflowNotice">
            日報23行を超える予定があります。納車 {dailyBoardModel.overflow.deliveries.length}件／入庫 {dailyBoardModel.overflow.inbound.length}件
          </div>
        )}
      </section>
    );
    ...
    <main className="page">
      ...
      <div className="dateNav noPrint">
        ...
        <div className="secondaryNavRow">
          ...
          <label className="layoutControl">状態<select value={completionPosition} onChange={(e) => changeCompletionPosition(e.target.value as CompletionPosition)}><option value="name">名前横</option><option value="meta">詳細欄</option></select></label>
        </div>
        <div className="quickNavRow">
          <button onClick={() => location.assign(`/schedule/week?day=${day}`)}>1週間</button>
          <button onClick={() => location.assign(`/schedule/month?day=${day}`)}>月</button>
          <button className="newEntry" onClick={() => location.assign(`/schedule/new?day=${day}`)}>＋予定</button>
          <button className="print" onClick={() => location.assign(`/schedule/print?day=${day}`)}>日報</button>
        </div>
      </div>

      {dailyReportStyleBoard()}
