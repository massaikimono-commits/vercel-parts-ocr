...
type ViewFilter = "all" | "notStarted" | "inProgress" | "urgent" | "unassigned";
...
export default function SchedulePage() {
  ...
  useEffect(() => {
    ...
    setFocusWorkId(focus);
    if (["all","notStarted","inProgress","urgent","unassigned"].includes(requestedFilter)) {
      setViewFilter(requestedFilter as ViewFilter);
    }
  ...
  function matchesViewFilter(work: WorkOrder | null) {
    if (viewFilter === "all") return true;
    if (!work) return false;
    if (viewFilter === "urgent") return work.is_urgent;
    if (viewFilter === "unassigned") return !work.worker_name?.trim();
    if (work.work_completed || work.status === "completed" || work.status === "cancelled") return false;
    if (viewFilter === "inProgress") return work.status === "in_progress";
    if (viewFilter === "notStarted") return work.status !== "in_progress";
    return true;
  }

  const filteredMorning = useMemo(
    () => morning.filter(({ work }) => matchesViewFilter(work)),
    [morning, viewFilter],
  );
  const filteredAfternoon = useMemo(
    () => afternoon.filter(({ work }) => matchesViewFilter(work)),
    [afternoon, viewFilter],
  );
  ...
  const filteredStayingVehicles = useMemo(
    () => stayingVehicles.filter(({ work }) => matchesViewFilter(work)),
    [stayingVehicles, viewFilter],
  );
  ...
  function workStateControl(work: WorkOrder | null) {
    ...
    if (work.work_completed || work.status === "completed") {
      return (
        <>
          <button className="workState completed noPrint" onClick={() => void toggleWorkCompleted(work)} aria-label="作業完了を解除">作業完了</button>
          <span className="printWorkState completed">作業完了</span>
        </>
      );
    }
    ...
    return (
      <>
        ...
        <button
          className="completeWorkNow noPrint"
          onClick={() => void toggleWorkCompleted(work)}
          aria-label="作業完了にする"
          title="作業完了"
        >
          完了
        ...
        <div className="itemTop">
          <div className="itemMain">
            ...
            <div className="scheduleSummary">
              ...
              <span className="entryTime">{entryLabel ? `${entryLabel} ` : ""}{timeLabel(entry)}</span>
            </div>
          </div>
  ...
  return (
    <main className="page">
      ...
      <div className="dateNav noPrint">
        ...
        <button onClick={() => location.assign(`/schedule/month?day=${day}`)}>1か月の予定検索</button>
      ...
      <section className="viewFilters noPrint" aria-label="予定の表示絞り込み">
        <button className={viewFilter==="all"?"active":""} onClick={()=>setViewFilter("all")}>すべて</button>
        <button className={viewFilter==="notStarted"?"active":""} onClick={()=>setViewFilter("notStarted")}>未実施</button>
        <button className={viewFilter==="inProgress"?"active":""} onClick={()=>setViewFilter("inProgress")}>作業中</button>
        <button className={viewFilter==="urgent"?"active":""} onClick={()=>setViewFilter("urgent")}>急ぎ</button>
        <button className={viewFilter==="unassigned"?"active":""} onClick={()=>setViewFilter("unassigned")}>担当未設定</button>
        {viewFilter!=="all" && <span>画面表示のみ絞り込み中</span>}
      </section>

      {section("午前", filteredMorning, "morning")}
      {section("午後", filteredAfternoon, "afternoon")}

      <section className="card staySection">
        <div className="sectionTitle"><h2>滞留車両</h2><span>{filteredStayingVehicles.length}台</span></div>
        {!filteredStayingVehicles.length && <div className="empty">{viewFilter==="all" ? "現在の滞留車両はありません。" : "この条件に合う滞留車両はありません。"}</div>}
        <div className="stayGrid">{filteredStayingVehicles.map(renderStayingVehicle)}</div>
      </section>
