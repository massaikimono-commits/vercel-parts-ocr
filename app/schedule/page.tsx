...
export default function SchedulePage() {
  ...
  function reasonClassName(work: WorkOrder | null) {
    ...
    if (work.reason === "一般整備") return work.outsource_vendor_name?.trim() ? "reason-body" : "reason-repair";
  ...
  function renderStayingVehicle(item: typeof stayingVehicles[number]) {
    ...
    const stayDays = stayDayCount(inboundDay, day) || 1;
    return (
      <article
        key={work.id}
        data-work-id={work.id}
        className={`stayItem ${reasonClassName(work)} ${work.is_urgent ? "urgentItem" : ""} ${focusWorkId === work.id ? "focusedWork" : ""}`}
        onClick={(event) => openScheduleDetailFromCard(event, inboundEntry)}
      >
        <div className="stayDesktopBody">
          ...
          <div className="meta">
            ...
            <span className={stayDays >= 3 ? "stayAge alert" : "stayAge"}>入庫 {stayDays}日目</span>
        ...
        <div className="stayMobileSummary">
          <div className="stayMobileMain">
            <b>{customerLabel(customer)}</b>
            <span>{last4Label(vehicle)}</span>
            <span>{work.reason}</span>
            <span className="mobileState">{workStateControl(work)}</span>
          </div>
          <div className="stayMobileSub">
            <span className={stayDays >= 3 ? "stayAgeText alert" : "stayAgeText"}>入庫{stayDays}日目</span>
            <span>滞留:{work.stay_reason || "未登録"}</span>
            <span>納車:未登録</span>
          </div>
          {(work.worker_name || work.outsource_vendor_name || vehicle?.maker || vehicle?.model) && (
            <div className="stayMobileExtra">
              {work.worker_name && <span>担当:{work.worker_name}</span>}
              {work.outsource_vendor_name && <span>外注:{work.outsource_vendor_name}</span>}
              {(vehicle?.maker || vehicle?.model) && <span>{[vehicle?.maker, vehicle?.model].filter(Boolean).join(" / ")}</span>}
            </div>
          )}
        </div>
