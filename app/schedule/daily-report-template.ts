...
export const DAILY_REPORT_TEMPLATE = {
  ...
  regions: {
    ...
    // "伝達事項" title stays on the pre-printed form; text starts to its right.
    messages: { x: 0.135, y: 0.726, width: 0.516, height: 0.054 },
    // Lower tables start below their printed header rows.
    stayingVehicles: { x: 0.044, y: 0.820, width: 0.334, height: 0.148 },
    // Stop before the vertical "納車予定車両" label column in the original PDF.
    bodyShopVehicles: { x: 0.378, y: 0.820, width: 0.273, height: 0.148 },
    // Planned-delivery data starts immediately below its printed column header.
    plannedDeliveries: { x: 0.681, y: 0.739, width: 0.290, height: 0.230 },
