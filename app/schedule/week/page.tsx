...
export default function WeeklySchedulePage() {
    ...
    try {
      ...
      if (workIds.length) {
        const { data, error } = await supabase
          .from("work_orders")
          .select("id,vehicle_id,reason,status,work_completed,is_urgent,needs_loaner,worker_name,outsource_vendor_name,is_waiting_service")
          .in("id", workIds);
  ...
  function overlapInfo(dayRows: EnrichedWeekRow[]) {
    let count = 0;
    ...
    <main className="weekPage">
      ...
      <section className={`weekBoard ${attentionOnly ? "attentionOnly" : ""}`}>
        ...
        {visibleWeekDays.map((day) => {
          ...
          const visitRows = dayRows.filter(({ entry, work }) =>
            entry.entry_type === "customer_visit" && work?.reason === "点検"
          );
          const visitTimes = visitRows.map(({ entry }) => dailyReportTimeLabel(entry));
          ...
          return (
            <article key={day} className={`dayColumn ${isToday ? "today" : ""} ${cap.className === "closed" ? "dayClosed" : ""}`}>
              ...
              {visitRows.length > 0 && (
                <button className="visitSummary" onClick={() => openDay(day)}>
