...
export default function WorkloadPage() {
  ...
  const rows = useMemo(() => {
    ...
    for (const work of works) {
      ...
      const row = map.get(name) || { name, total: 0, notStarted: 0, inProgress: 0, completedWaiting: 0 };
      row.total += 1;
    ...
    return [...map.values()].sort((a, b) => {
      ...
      return bActive - aActive || b.total - a.total || a.name.localeCompare(b.name, "ja");
    });
  }, [works]);
  ...
  }), { total: 0, notStarted: 0, inProgress: 0, completedWaiting: 0 }), [rows]);
    ...
    <main className="loadPage">
      ...
      <section className="tableCard">
        <div className="tableHead">
          <span>担当者</span><span>未実施</span><span>作業中</span><span>完了待ち</span><span>合計</span>
        </div>
        {rows.map((row) => (
      ...
      <div className="hint">同じ作業を複数予定に登録していても、work_orders単位で1台として集計します。作業完了後も出庫前なら「完了待ち」に残ります。</div>
