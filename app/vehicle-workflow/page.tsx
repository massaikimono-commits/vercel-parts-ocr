...
function extractCertificate(reads: CertificateReads): Vehicle {
  ...
  const firstRegistration = parseJapaneseMonth(reads.firstRegistration) || "";
...
export default function VehicleWorkflowPage() {
  ...
  useEffect(() => { (async () => { try {
    ...
    const list = (data || []).map((v: any) => ({ id: v.id, number: v.vehicle_number || "", registration: v.registration_number || "", last4: v.registration_number_last4 || "", chassis: v.chassis_number || "", model: v.model || "", type: (v.fuel_type || "その他") as Vehicle["type"], weight: v.vehicle_weight == null ? "" : String(v.vehicle_weight), firstRegistration: v.first_registration || "", customerId: v.customer_id || "" }));
  ...
  async function saveVehicle() {
    ...
    const saved: Vehicle = { ...vehicle, id: data.id, number: payload.vehicle_number, last4: payload.registration_number_last4 || "" }; setVehicle(saved); setVehicles((old) => [saved, ...old.filter((x) => x.id !== saved.id && x.number !== saved.number)]); localStorage.setItem(ACTIVE_KEY, JSON.stringify(saved)); setMessage("車両情報を保存し、作業車両に設定しました。");
  }
