...
export default function VehicleWorkflowPage() {
  ...
  return <main className="page">
    ...
    <section className="card certificate"><h2>車検証から車両情報を読み取る</h2><p>車検証の決まった位置を項目ごとに拡大して、登録番号・車台番号・型式・燃料・車両重量・初度登録年月を別々に読み取ります。初度登録は車検証どおり和暦で保存します。</p><input ref={cameraRef} className="hidden" type="file" accept="image/*" capture="environment" onChange={(e) => { const f = e.target.files?.[0]; if (f) void readVehicleCertificate(f); e.currentTarget.value = ""; }} /><input ref={libraryRef} className="hidden" type="file" accept="image/*" onChange={(e) => { const f = e.target.files?.[0]; if (f) void readVehicleCertificate(f); e.currentTarget.value = ""; }} /><div className="docActions"><button className="primaryDoc" disabled={docBusy} onClick={() => cameraRef.current?.click()}>📷 今撮影して読み取る</button><button disabled={docBusy} onClick={() => libraryRef.current?.click()}>🖼 写真ライブラリから読み取る</button></div>{docBusy && <div className="progress"><div style={{ width: `${docProgress}%` }} /></div>}{docPreview && <img className="preview" src={docPreview} alt="車検証プレビュー" />}{docText && <details><summary>OCR詳細（確認用）</summary><pre>{docText}</pre></details>}</section>
