...
const EVAL_BRANCH =
  "eval/parts-ocr-stage-a11-anchored-alt-lattice";
const EVAL_HEAD =
  process.env.NEXT_PUBLIC_EVAL_HEAD || "unknown-preview-head";
const REVISION =
  "stage-a11-anchored-alt-lattice-v1";
...
function buildManagementShortSummary(source: any) {
  ...
  const importantImages = (source?.images || [])
    ...
    .map((image: any) => ({
      ...
      A10_G: image.variants?.A10_G_reference?.gtCoverage,
      L: image.variants?.L_anchoredLattice?.gtCoverage,
      L_C: image.variants?.L_C_topologyAcceptance?.gtCoverage,
      Danchors: image.anchorTrace?.anchorCount,
      pitch: image.anchorTrace?.pitch,
      columnLaneCount: image.columnLaneCount,
      rowLikeCandidateCount: image.rowLikeCandidateCount,
      Laccepted: image.variants?.L_anchoredLattice?.altCandidateAccepted,
      L_Caccepted: image.variants?.L_C_topologyAcceptance?.altCandidateAccepted,
    }));
  ...
  const summary = {
    ...
    a9AltReference: source?.a9AltReference,
    a10GReference: source?.a10GReference,
...
export default function StageA11Page() {
  ...
  async function run(files: FileList | null) {
    ...
    try {
      ...
      for (let i = 0; i < EXPECTED_FILES.length; i += 1) {
        ...
        setStatus(`${i + 1}/12 ${fileName} Stage A11解析中…`);
        ...
        const latticeLC = autoAbnormal.eligible
          ? growAnchoredAltLattices({
              dRows,
              altRows: topology.rows,
              rawLines: lineBuild.lines,
        ...
        const lcAltRows = latticeLC.selection.acceptedCandidateIndexes.map(
          (index) => altNovel.rows[index - 1],
        );
        ...
        const lcRows: CandidateRow[] = [
          ...asRows(dRows),
          ...lcAltRows.map((row: any) => ({
            top: row.top,
            bottom: row.bottom,
            center: row.center,
            source: "alt-lattice-topology",
          })),
        ].sort((a, b) => a.center - b.center);
        ...
        const lcScore = scoreRows(lcRows, paper, meta, gt);
        ...
        const variants = {
          dBaseline: variantStats(dScore, 0, null),
          ...
          L_anchoredLattice: variantStats(
            lScore,
            lAltRows.length,
            latticeL.selection,
          ),
          L_C_topologyAcceptance: variantStats(
            lcScore,
            lcAltRows.length,
            latticeLC.selection,
          ),
        ...
        images.push({
          ...
          anchorTrace: latticeL.trace,
          topologyAnchorTrace: latticeLC.trace,
          latticeTrace: latticeL.selection,
          topologyLatticeTrace: latticeLC.selection,
          variants,
        });
      }
      ...
      const aggregate = {
        L_anchoredLattice: summarizeVariant(
          images,
          "L_anchoredLattice",
        ),
        L_C_topologyAcceptance: summarizeVariant(
          images,
          "L_C_topologyAcceptance",
        ),
      };

      const mechanismAggregate = {
        L_anchoredLattice:
          aggregate.L_anchoredLattice.mechanismRescue,
        L_C_topologyAcceptance:
          aggregate.L_C_topologyAcceptance.mechanismRescue,
      };

      const payload = {
        schema:
          "icb.parts-ocr.stage-a11-anchored-alt-lattice.v1",
        revision: REVISION,
        ...
        experimentOnly: true,
        gtUsage:
          "post-hoc scoring/mechanism attribution only; no GT lattice generation, pitch, lane clustering, ranking, acceptance, envelope, or control",
        baseline: {
          ...baseline,
      ...
      setResult(payload);
      setStatus(
        "12/12 Stage A11解析完了。総合管理用短縮summaryをコピーできます。",
      );
    } catch (error) {
      setStatus(
    ...
    <main style={styles.page}>
      <section style={styles.card}>
        <h1 style={styles.title}>
          Stage A11 Anchored ALT Row-Lattice
        </h1>
        <p style={styles.text}>
          A5-D / A9 ALT / A10 Gを同runで再現し、
          L（anchored ALT lattice）と
          L+C（multi-column topology ranking / acceptance）を比較します。
          D-only missing-slot generation、targeted split、新PSMは使用しません。
        </p>
      </section>
