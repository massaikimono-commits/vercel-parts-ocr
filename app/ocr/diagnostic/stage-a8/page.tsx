...
import {
  ...
  summarizeDocumentBandGeometry,
  traceConsolidationAndPreservation,
  traceWordFallbackEligibility,
} from "./structure-trace";
...
function stageHasBandOverlap(
  stage: any,
  gtRow: { y1Norm: number; y2Norm: number },
) {
...
function normalizedOverlap(
  ...
  const gtHeight = Math.max(1e-9, gtRow.y2Norm - gtRow.y1Norm);
  const candidateHeight = Math.max(
    1e-9,
    candidate.y2Norm - candidate.y1Norm,
  );

  return {
    overlapNorm: overlap,
    overlapRatioToGt: overlap / gtHeight,
    overlapRatioToCandidate: overlap / candidateHeight,
  };
}

function nearestMappedCandidate(
  mapped: MappedCandidate[],
  gtRow: { y1Norm: number; y2Norm: number },
) {
  if (!mapped.length) return null;

  const gtCenter = (gtRow.y1Norm + gtRow.y2Norm) / 2;

  return [...mapped].sort(
    (a, b) =>
      Math.abs(a.centerYNorm - gtCenter) -
      Math.abs(b.centerYNorm - gtCenter),
  )[0];
}
...
export default function StageA8Page() {
  ...
  async function run(files: FileList | null) {
    ...
    try {
      ...
      for (let i = 0; i < EXPECTED_FILES.length; i += 1) {
        ...
        const structure = traceConsolidationAndPreservation({
          rawLines: lineBuild.lines,
          consolidated: consolidated.rows,
          wordGroups: wordGroups.rows,
        });

        const fallbackStructure = traceWordFallbackEligibility({
        ...
        const stages = {
          ...
          consolidated: scoreRows(
            asRows(consolidated.rows),
            paper,
            meta,
            gt,
          ),
          gapSelective: scoreRows(
        ...
        const documentBandGeometry =
          summarizeDocumentBandGeometry(dRows);

        const missRows = gt.rows
          .filter(
            (row) =>
              !stages.dBaseline.coveredGtRows.includes(row.rowIndex),
          )
          .map((gtRow) => {
            ...
            const mechanism =
              classification.nonPathologicalMechanism;

            let bDiagnostic: any = null;
            let dDiagnostic: any = null;

            if (mechanism === "B") {
              const rawHits = stages.rawTsvLine.mapped.filter(
                (candidate: MappedCandidate) =>
                  candidate.matchedGtRowIndex === gtRow.rowIndex,
              );

              bDiagnostic = rawHits.map(
                (candidate: MappedCandidate) => {
                  const rawTrace =
                    structure.preservation.rows[
                      candidate.candidateIndex - 1
                    ];

                  const group =
                    structure.consolidation.groups.find(
                      (item: any) =>
                        item.memberDiagnosticIds.includes(
                          rawTrace?.rawDiagnosticId,
                        ),
                    ) || null;

                  return {
                    rawLineDiagnosticId:
                      rawTrace?.rawDiagnosticId ?? null,
                    rawCenter: rawTrace?.rawCenter ?? null,
                    rawHeight: rawTrace?.rawHeight ?? null,
                    mergeTargetConsolidatedId:
                      rawTrace?.mergeTargetConsolidatedId ?? null,
                    consolidationGroup: group,
                    wordGroupSupport:
                      rawTrace?.wordGroupSupport ?? null,
                    level5ChildCount:
                      rawTrace?.level5ChildCount ?? null,
                    preservationPredicate:
                      rawTrace?.preservationPredicate ?? null,
                    preserveAccepted:
                      rawTrace?.preserveAccepted ?? null,
                    preserveRejectedBy:
                      rawTrace?.preserveRejectedBy ?? null,
                  };
                },
              );
            }
            ...
            if (mechanism === "D") {
              const nearest = nearestMappedCandidate(
                stages.dBaseline.mapped,
                gtRow,
              );

              if (nearest) {
                const pixelCandidate =
                  dRows[nearest.candidateIndex - 1];

                const support = candidateSupportSummary({
                  candidate: pixelCandidate,
                  rawLines: lineBuild.lines,
                  wordGroups: wordGroups.rows,
                });

                const overlap = normalizedOverlap(
                  nearest,
                  gtRow,
                );

                const gtCentersInsideCandidateBand = gt.rows
                  .filter((row) => {
                    const center =
                      (row.y1Norm + row.y2Norm) / 2;
                    return (
                      center >= nearest.y1Norm &&
                      center <= nearest.y2Norm
                    );
                  })
                  .map((row) => row.rowIndex);

                const candidateHeightPx =
                  pixelCandidate.bottom -
                  pixelCandidate.top +
                  1;

                const candidateHeightToDocumentMedian =
                  candidateHeightPx /
                  documentBandGeometry.medianCandidateHeight;
                ...
                dDiagnostic = {
                  ...
                  nearestCandidateSource:
                    nearest.source,
                  nearestCandidateCenterYNorm:
                    nearest.centerYNorm,
                  candidateY1Norm: nearest.y1Norm,
                  candidateY2Norm: nearest.y2Norm,
                  candidateHeightNorm:
                    nearest.y2Norm - nearest.y1Norm,
                  candidateHeightPx,
                  documentMedianCandidateHeightPx:
                    documentBandGeometry.medianCandidateHeight,
                  candidateHeightToDocumentMedian,
                  broadBandByDocumentGeometry:
                    candidateHeightToDocumentMedian >= 2,
                  gtY1Norm: gtRow.y1Norm,
                  gtY2Norm: gtRow.y2Norm,
                  gtCenterYNorm:
                    (gtRow.y1Norm + gtRow.y2Norm) / 2,
                  centerDistanceNorm:
                    Math.abs(
                      nearest.centerYNorm -
                        (gtRow.y1Norm + gtRow.y2Norm) / 2,
                  ...
                  ...overlap,
                  rawLineSupport: support.rawLineSupport,
                  wordGroupSupport:
                    support.wordGroupSupport,
                  gtRowCentersInsideCandidateBand:
                    gtCentersInsideCandidateBand,
                  spansMultipleGtRowCenters:
                    gtCentersInsideCandidateBand.length >= 2,
                };
              }
            }
        ...
        const a4LineNewRescueRows =
          stages.rawTsvLine.coveredGtRows.filter(
            (rowIndex: number) =>
              !stages.rules.coveredGtRows.includes(rowIndex),
          );

        const lostA4RescueRows =
          a4LineNewRescueRows.filter(
            (rowIndex: number) =>
              !stages.dBaseline.coveredGtRows.includes(rowIndex),
          );

        const lostA4RescueDiagnostics =
          lostA4RescueRows.map((rowIndex: number) => {
            ...
            return {
              ...
              bDiagnostic: miss?.bDiagnostic ?? null,
            };
          });
        ...
        for (const miss of missRows) {
          const key = miss.classification as keyof typeof mechanismCounts;
          mechanismCounts[key] += 1;
        }

        const aMissCount = missRows.filter(
          (row: any) =>
            row.nonPathologicalMechanism === "A",
        ).length;
        ...
        images.push({
          ...
          pipelineCounts: {
            ...
            wordGroupCount: wordGroups.rows.length,
            rawTsvLineCount: rawLineRows.length,
            consolidatedCount:
              consolidated.rows.length,
            gapSelectiveRescueCount:
              gapSelective.rows.length,
          },
          ...
          a4LineNewRescueRows,
          lostA4RescueRows,
          lostA4RescueDiagnostics,
          bStructureTrace:
            lostA4RescueRows.length ||
            missRows.some(
              (row: any) =>
                row.nonPathologicalMechanism === "B",
            )
              ? {
                  consolidation:
              ...
              : null,
          fallbackStructureTrace:
            aMissCount > 0 || pathological
              ? fallbackStructure
              : null,
          documentBandGeometry,
        });
      }
      ...
      const dBaseline = {
        candidateCount: images.reduce(
          (sum, image) =>
            sum + image.dBaseline.candidateCount,
          0,
        ),
        gtCoverage: images.reduce(
          (sum, image) =>
            sum + image.dBaseline.gtCoverage,
          0,
        ),
        falseCount: images.reduce(
          (sum, image) =>
            sum + image.dBaseline.falseCount,
          0,
        ),
        duplicateCount: images.reduce(
          (sum, image) =>
            sum + image.dBaseline.duplicateCount,
          0,
        ),
      };
      ...
      for (const image of images) {
        for (const key of Object.keys(
          ...
          keyof typeof aggregateClassifications
        >) {
          aggregateClassifications[key] +=
            image.missClassificationCounts[key];
        }
      }

      const lostA4RescueRows = images.flatMap(
        (image) =>
          image.lostA4RescueDiagnostics.map(
            (row: any) => ({
      ...
      const payload = {
        schema:
          "icb.parts-ocr.stage-a8-structure-diagnostic.v1",
        sourceHead: SOURCE_HEAD,
        evaluationSet: "formal-yellow-12",
        gtTotalRows: 54,
        experimentOnly: true,
        gtUsage:
          "post-hoc scoring and diagnostic attribution only; no GT candidate generation, selection, filtering, thresholding, fallback eligibility, or control",
        dBaseline: {
          ...dBaseline,
          ...
          matchesExpectedA5D:
            dBaseline.candidateCount ===
              EXPECTED_A5_D.candidateCount &&
            dBaseline.gtCoverage ===
              EXPECTED_A5_D.gtCoverage &&
            dBaseline.falseCount ===
              EXPECTED_A5_D.falseCount &&
            dBaseline.duplicateCount ===
              EXPECTED_A5_D.duplicateCount &&
            bothMiss ===
              EXPECTED_A5_D.bothMiss,
        },
        missClassification:
          aggregateClassifications,
      ...
      setResult(payload);
      setStatus(
        "12/12 Stage A8構造診断完了。summaryをコピーできます。",
      );
    } catch (error) {
      setStatus(
  ...
  function shortSummary() {
    ...
    return {
      ...
      missClassification:
        result.missClassification,
      pathologicalTsvImages:
        result.pathologicalTsvImages,
      lostA4RescueRows:
        result.lostA4RescueRows,
      images: result.images
        .filter(
          (image: any) =>
            image.missRows.length > 0,
        )
        .map((image: any) => ({
          fileName: image.fileName,
          ...
          bStructureTrace:
            image.bStructureTrace,
          fallbackStructureTrace:
            image.fallbackStructureTrace,
          documentBandGeometry:
            image.documentBandGeometry,
        })),
    };
  }
    ...
    <main style={styles.page}>
      <section style={styles.card}>
        <h1 style={styles.title}>
          Stage A8 Structure Diagnostic
        </h1>

        <p style={styles.text}>
          A5-Dを固定baselineとして、
          Stage A7が狙ったB / D / A-fallback構造を
          trace-onlyで観測します。
          新救済candidate、parameter変更、
          localization書換え、alternate segmentationは追加しません。
        </p>
      </section>
