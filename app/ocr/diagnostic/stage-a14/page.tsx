...
} from "../stage-a13/loss-decomposition";
import {
  selectLatePhaseReconsideration,
} from "./late-reconsideration";
...
const EVAL_BRANCH =
  "eval/parts-ocr-stage-a14-late-phase-reconsideration";
const EVAL_HEAD =
  process.env.NEXT_PUBLIC_EVAL_HEAD || "unknown-preview-head";
const REVISION =
  "stage-a14-late-phase-reconsideration-v1";
...
function buildManagementShortSummary(source: any) {
  ...
  const importantImages = (source?.images || [])
    ...
    .map((image: any) => ({
      ...
      P: image.variants?.P_CONTROL?.gtCoverage,
      A14_R: image.variants?.A14_R_LATE_RECONSIDERATION?.gtCoverage,
      rowLikeCandidateCount: image.rowLikeCandidateCount,
      acceptedALTCount:
        image.variants?.P_CONTROL?.altCandidateAccepted,
      ...
      lateCandidateConsidered:
        image.lateSelection?.lateCandidateConsidered,
      lateCandidateAccepted:
        image.lateSelection?.lateCandidateAccepted,
      ...
      lostRows:
        (image.lossDiagnostic?.rows || []).map((row: any) => ({
          ...
          reason: row.reason,
          recovered:
            image.recoveredLostRows?.includes(row.rowIndex) || false,
        })),
    }));
  ...
  const summary = {
    ...
    referenceValidity: {
      ...
      A12P: source?.pControl?.matchesExpectedA12P,
    },
    P_CONTROL: source?.pControl,
    A14_R_LATE_RECONSIDERATION:
      source?.a14R,
    lateCandidateConsidered: source?.lateCandidateConsidered,
    lateCandidateAccepted: source?.lateCandidateAccepted,
    lateCandidateCorrectPostHoc:
      source?.lateCandidateCorrectPostHoc,
    lateCandidateFalsePostHoc:
      source?.lateCandidateFalsePostHoc,
    recoveredLostRows: source?.recoveredLostRows,
    remainingLostRowsByReason:
      source?.remainingLostRowsByReason,
    allLostRows: source?.allLostRows,
    importantImages,
  ...
  const compact = {
    ...
    evaluationBranch: summary.evaluationBranch,
    evaluationHead: summary.evaluationHead,
    frozenSourceHead: summary.frozenSourceHead,
    referenceValidity: summary.referenceValidity,
    P_CONTROL: source?.pControl
      ? {
          candidateCount: source.pControl.candidateCount,
          gtCoverage: source.pControl.gtCoverage,
          falseCount: source.pControl.falseCount,
          duplicateCount: source.pControl.duplicateCount,
          newRescueVsD: source.pControl.newRescueVsD,
          retainedA9AltRescue: source.pControl.retainedA9AltRescue,
          correctRowRegressionVsD:
            source.pControl.correctRowRegressionVsD,
        }
      : null,
    A14_R_LATE_RECONSIDERATION: source?.a14R
      ? {
          candidateCount: source.a14R.candidateCount,
          gtCoverage: source.a14R.gtCoverage,
          falseCount: source.a14R.falseCount,
          duplicateCount: source.a14R.duplicateCount,
          newRescueVsD: source.a14R.newRescueVsD,
          retainedA9AltRescue: source.a14R.retainedA9AltRescue,
          correctRowRegressionVsD:
            source.a14R.correctRowRegressionVsD,
        }
      : null,
    lateCandidateConsidered: source?.lateCandidateConsidered,
    lateCandidateAccepted: source?.lateCandidateAccepted,
...
export default function StageA14Page() {
  ...
  async function run(files: FileList | null) {
    ...
    try {
      ...
      for (let i = 0; i < EXPECTED_FILES.length; i += 1) {
        ...
        setStatus(`${i + 1}/12 ${fileName} Stage A14実験中…`);
        ...
        const lateSelection = selectLatePhaseReconsideration({
          topologyRows: topology.rows,
          phaseResult: phaseP,
        });

        const lateRows = lateSelection.acceptedCandidateIndexes.map(
          (index) => altNovel.rows[index - 1],
        );

        const a14Rows: CandidateRow[] = [
          ...pRows,
          ...lateRows.map((row: any) => ({
            top: row.top,
            bottom: row.bottom,
            center: row.center,
            source: "alt-late-phase-reconsideration",
          })),
        ].sort((a, b) => a.center - b.center);
        ...
        const a14Score = scoreRows(a14Rows, paper, meta, gt);
        const lateOnlyScore = scoreRows(
          lateRows.map((row: any) => ({
            top: row.top,
            bottom: row.bottom,
            center: row.center,
        ...
        const pControlStats = variantStats(
          pScore,
          pAltRows.length,
          null,
          phaseP,
        );
        const a14Stats = variantStats(
          a14Score,
          pAltRows.length + lateRows.length,
          null,
          phaseP,
        );

        const recoveredLostRows = intersection(
          lostCorrectRows,
          a14Score.coveredGtRows,
        );

        const lateCorrectCandidateCount = lateOnlyScore.mapped.filter(
          (candidate: MappedCandidate) =>
            candidate.matchedGtRowIndex != null,
        ).length;
        ...
        const variants = {
          ...
          P_CONTROL: pControlStats,
          A14_R_LATE_RECONSIDERATION: a14Stats,
        };
        ...
        images.push({
          ...
          pSuccessDiagnostic,
          lateSelection,
          lateCandidateCorrectPostHoc: lateCorrectCandidateCount,
          lateCandidateFalsePostHoc: lateOnlyScore.falseCount,
          variants,
        });
      }
      ...
      const pControl = summarizeVariant(
        images,
        "P_CONTROL",
      );
      const a14R = summarizeVariant(
        images,
        "A14_R_LATE_RECONSIDERATION",
      );
      ...
      const remainingLostRowsByReason = {
        NO_ALT_CANDIDATE: 0,
        NOT_ROW_LIKE: 0,
        PITCH_HYPOTHESIS_MISS: 0,
        PHASE_RESIDUAL_REJECT: 0,
      ...
      const allLostRows: any[] = [];
      const recoveredLostRows: any[] = [];
      let lateCandidateConsidered = 0;
      let lateCandidateAccepted = 0;
      let lateCandidateCorrectPostHoc = 0;
      let lateCandidateFalsePostHoc = 0;

      for (const image of images) {
        lateCandidateConsidered +=
          image.lateSelection?.lateCandidateConsidered || 0;
        lateCandidateAccepted +=
          image.lateSelection?.lateCandidateAccepted || 0;
        lateCandidateCorrectPostHoc +=
          image.lateCandidateCorrectPostHoc || 0;
        lateCandidateFalsePostHoc +=
          image.lateCandidateFalsePostHoc || 0;
        ...
        for (const row of image.lossDiagnostic?.rows || []) {
          const recovered =
            image.recoveredLostRows?.includes(row.rowIndex) || false;
          ...
          const item = {
            fileName: image.fileName,
            rowIndex: row.rowIndex,
            a13Reason: row.reason,
            recovered,
          };

          allLostRows.push(item);
          if (recovered) {
            recoveredLostRows.push(item);
          } else {
            remainingLostRowsByReason[
              row.reason as keyof typeof remainingLostRowsByReason
            ] += 1;
          }
        }
      }
      ...
      const payload = {
        schema:
          "icb.parts-ocr.stage-a14-late-phase-reconsideration.v1",
        revision: REVISION,
        ...
        experimentOnly: true,
        gtUsage:
          "post-hoc scoring/recovery attribution only; no GT OCR, proposal generation, rowLike, pitch, phase, assignment, late selection, ranking, acceptance, dedupe, stop, or control",
        baseline: {
          ...baseline,
        ...
        pControl: {
          ...
          matchesExpectedA12P:
            pControl.gtCoverage === EXPECTED_A12_P.gtCoverage &&
            pControl.falseCount === EXPECTED_A12_P.falseCount &&
            pControl.duplicateCount === EXPECTED_A12_P.duplicateCount &&
            pControl.newRescueVsD === EXPECTED_A12_P.newRescueVsD &&
            pControl.retainedA9AltRescue ===
              EXPECTED_A12_P.retainedA9AltRescue &&
            pControl.correctRowRegressionVsD ===
              EXPECTED_A12_P.correctRowRegressionVsD,
        },
        a14R,
        a13LossReasonAggregate: lossReasonAggregate,
        lateCandidateConsidered,
        lateCandidateAccepted,
        lateCandidateCorrectPostHoc,
        lateCandidateFalsePostHoc,
      ...
      setResult(payload);
      setStatus(
        "12/12 Stage A14実験完了。総合管理用短縮summaryをコピーできます。",
      );
    } catch (error) {
      setStatus(
    ...
    <main style={styles.page}>
      <section style={styles.card}>
        <h1 style={styles.title}>
          Stage A14 Late Phase Reconsideration
        </h1>
        <p style={styles.text}>
          A12 P_phaseLockedをP_CONTROLとして完全確定した後、
          未割当phase positionだけに対し、既存altNovelの
          rowLike=false candidateをlate reconsiderationします。
          Pのselected pitch / phase / phase position /
          assignment winner / accepted rowは変更しません。
          P_SELF・新PSM・新OCR・新preprocessingは使用しません。
        </p>
      </section>
