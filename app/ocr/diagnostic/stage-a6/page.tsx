...
function mapCandidate(
  ...
  const x1 = paper.x;
  const x2 = paper.x + paper.w;
  const y1 = row.top;
  const y2 = row.bottom + 1;
  ...
  let match: null | (typeof gt.rows)[number] = null;
...
function stageHasBandOverlap(stage: any, gtRow: { y1Norm: number; y2Norm: number }) {
...
function nearestDistance(stage: any, gtRow: { y1Norm: number; y2Norm: number }) {
  if (!stage.mapped.length) return null;
  const center = (gtRow.y1Norm + gtRow.y2Norm) / 2;
  return Math.min(
    ...stage.mapped.map((candidate: MappedCandidate) =>
      Math.abs(candidate.centerYNorm - center),
    ),
  );
}

function classifyMiss(args: {
  gtRow: { rowIndex: number; y1Norm: number; y2Norm: number };
  rules: any;
  rawLine: any;
  consolidated: any;
  gapSelective: any;
  ...
  let nonPathologicalMechanism = "F";

  if (rawHit && !consolidatedHit) {
    nonPathologicalMechanism = "B";
  } else if (consolidatedHit && !gapHit) {
    nonPathologicalMechanism = "C";
  } else if (!ruleHit && !rawHit && anyOverlap) {
    nonPathologicalMechanism = "D";
  } else if (!ruleHit && !rawHit && !anyOverlap) {
    nonPathologicalMechanism = "A";
  } else {
    nonPathologicalMechanism = "F";
  }

  const classification = pathological ? "E" : nonPathologicalMechanism;

  return {
    classification,
    nonPathologicalMechanism,
    stageFacts: {
      ruleCenterHit: ruleHit,
      rawTsvLineCenterHit: rawHit,
      consolidatedCenterHit: consolidatedHit,
      gapSelectiveCenterHit: gapHit,
      anyBandOverlap: anyOverlap,
      nearestRuleCenterDistanceNorm: nearestDistance(rules, gtRow),
      nearestRawLineCenterDistanceNorm: nearestDistance(rawLine, gtRow),
      nearestConsolidatedCenterDistanceNorm: nearestDistance(
        consolidated,
        gtRow,
      ),
      nearestGapSelectiveCenterDistanceNorm: nearestDistance(
        gapSelective,
        gtRow,
      ),
    },
...
export default function StageA6Page() {
  ...
  async function run(files: FileList | null) {
    ...
    try {
      ...
      for (let i = 0; i < EXPECTED_FILES.length; i += 1) {
        ...
        const parsed = parseHeaderlessRuntimeTsv(
          String(full.data.tsv || ""),
        );
        ...
        const consolidated = consolidateLineCandidates(
          lineBuild.lines,
        );
        ...
        const dRows = combineRulesAndRescue(
          ruleRows,
          gapSelective.rows,
        );
        ...
        const stages = {
          rules: scoreStage(asRows(ruleRows), paper, meta, gt),
          rawTsvLine: scoreStage(asRows(rawLineRows), paper, meta, gt),
          consolidatedTsvLine: scoreStage(
            asRows(consolidated.rows),
            paper,
            meta,
            gt,
          ),
          gapSelectiveTsvRescue: scoreStage(
            asRows(gapSelective.rows),
            paper,
            meta,
            gt,
          ),
          dBaseline: scoreStage(asRows(dRows), paper, meta, gt),
        };
        ...
        const maxWordHeight = wordHeights.length
          ? Math.max(...wordHeights)
          : 0;

        const pathologicalTsvSegmentation =
          lineBuild.trace.level5WordCountInsidePaper <= 1 &&
          maxWordHeight > paper.h * 0.5;

        const dMissRows = gt.rows.filter(
          (row) => !stages.dBaseline.coveredGtRows.includes(row.rowIndex),
        );

        const missRows = dMissRows.map((gtRow) => {
          const classified = classifyMiss({
            gtRow,
            rules: stages.rules,
            rawLine: stages.rawTsvLine,
            consolidated: stages.consolidatedTsvLine,
            gapSelective: stages.gapSelectiveTsvRescue,
            pathological: pathologicalTsvSegmentation,
          });
          ...
          return {
            ...
            gtCenterYNorm: (gtRow.y1Norm + gtRow.y2Norm) / 2,
            ...classified,
          };
        });

        const a4LineNewRescueRows = stages.rawTsvLine.coveredGtRows.filter(
          (rowIndex: number) => !stages.rules.coveredGtRows.includes(rowIndex),
        );

        const lostA4RescueRows = a4LineNewRescueRows
          .filter(
            (rowIndex: number) =>
              !stages.dBaseline.coveredGtRows.includes(rowIndex),
          )
          .map((rowIndex: number) => {
            const miss = missRows.find(
              (row: any) => row.gtRowIndex === rowIndex,
            );

            return {
              gtRowIndex: rowIndex,
              classification: miss?.classification || "F",
              nonPathologicalMechanism:
                miss?.nonPathologicalMechanism || "F",
        ...
        for (const miss of missRows) {
          classCounts[
            miss.classification as keyof typeof classCounts
          ] += 1;
        }
        ...
        images.push({
          ...
          pathologicalTsvSegmentation,
          pipelineTrace: {
            runtimeTsvRecordCount: parsed.contract.recordCount,
            runtimeTsvValidRecordCount: parsed.contract.validRecordCount,
            level4LineCountInsidePaper:
              lineBuild.trace.level4LineCountInsidePaper,
            level5WordCountInsidePaper:
              lineBuild.trace.level5WordCountInsidePaper,
            rawTsvLineCandidateCount: rawLineRows.length,
            consolidatedTsvCandidateCount: consolidated.rows.length,
            gapSelectiveTsvRescueCount: gapSelective.rows.length,
            wordGroupCandidateCount: wordGroup.rows.length,
          },
          dBaseline: {
            candidateCount: stages.dBaseline.candidateCount,
            gtCoverage: stages.dBaseline.gtCoverage,
            recall: stages.dBaseline.recall,
            falseCount: stages.dBaseline.falseCount,
            duplicateCount: stages.dBaseline.duplicateCount,
            coveredGtRows: stages.dBaseline.coveredGtRows,
          },
          stageCoverage: {
            rules: {
              candidateCount: stages.rules.candidateCount,
              gtCoverage: stages.rules.gtCoverage,
              falseCount: stages.rules.falseCount,
              duplicateCount: stages.rules.duplicateCount,
              coveredGtRows: stages.rules.coveredGtRows,
            },
            rawTsvLine: {
              candidateCount: stages.rawTsvLine.candidateCount,
              gtCoverage: stages.rawTsvLine.gtCoverage,
              falseCount: stages.rawTsvLine.falseCount,
              duplicateCount: stages.rawTsvLine.duplicateCount,
              coveredGtRows: stages.rawTsvLine.coveredGtRows,
            },
            consolidatedTsvLine: {
              candidateCount: stages.consolidatedTsvLine.candidateCount,
              gtCoverage: stages.consolidatedTsvLine.gtCoverage,
              falseCount: stages.consolidatedTsvLine.falseCount,
              duplicateCount: stages.consolidatedTsvLine.duplicateCount,
              coveredGtRows: stages.consolidatedTsvLine.coveredGtRows,
            },
            gapSelectiveTsvRescue: {
              candidateCount: stages.gapSelectiveTsvRescue.candidateCount,
              gtCoverage: stages.gapSelectiveTsvRescue.gtCoverage,
              falseCount: stages.gapSelectiveTsvRescue.falseCount,
              duplicateCount: stages.gapSelectiveTsvRescue.duplicateCount,
              coveredGtRows: stages.gapSelectiveTsvRescue.coveredGtRows,
            },
          },
          missClassificationCounts: classCounts,
          missRows,
          a4LineNewRescueRows,
          lostA4RescueRows,
        });
      ...
      for (const image of images) {
        ...
        ) as Array<keyof typeof aggregateClassification>) {
          aggregateClassification[key] +=
            image.missClassificationCounts[key];
        }
      }

      const dBaseline = {
        candidateCount: images.reduce(
          (sum, image) => sum + image.dBaseline.candidateCount,
          0,
        ),
        gtCoverage: images.reduce(
          (sum, image) => sum + image.dBaseline.gtCoverage,
          0,
        ),
        falseCount: images.reduce(
          (sum, image) => sum + image.dBaseline.falseCount,
          0,
        ),
        duplicateCount: images.reduce(
          (sum, image) => sum + image.dBaseline.duplicateCount,
          0,
        ),
      };

      const bothMiss = 54 - dBaseline.gtCoverage;

      const lostA4RescueRows = images.flatMap((image) =>
        image.lostA4RescueRows.map((row: any) => ({
      ...
      const payload = {
        schema:
          "icb.parts-ocr.stage-a6-row-miss-mechanism.v1",
        sourceHead: SOURCE_HEAD,
        evaluationSet: "formal-yellow-12",
        gtTotalRows: 54,
        experimentOnly: true,
        gtUsage:
          "post-hoc scoring/classification only; no GT candidate generation, filtering, thresholding, or selection",
        dBaseline: {
          ...dBaseline,
          recall: dBaseline.gtCoverage / 54,
          bothMiss,
          expectedA5D: EXPECTED_A5_D,
          matchesExpectedA5D:
            dBaseline.candidateCount === EXPECTED_A5_D.candidateCount &&
            dBaseline.gtCoverage === EXPECTED_A5_D.gtCoverage &&
            dBaseline.falseCount === EXPECTED_A5_D.falseCount &&
            dBaseline.duplicateCount === EXPECTED_A5_D.duplicateCount &&
            bothMiss === EXPECTED_A5_D.bothMiss,
        },
        missClassification: aggregateClassification,
      ...
      setResult(payload);
      setStatus(
        "12/12 Stage A6診断完了。summaryをコピーできます。",
      );
    } catch (error) {
      setStatus(
  ...
  function shortSummary() {
    ...
    return {
      ...
      dBaseline: result.dBaseline,
      missClassification: result.missClassification,
      pathologicalTsvImages: result.pathologicalTsvImages,
      lostA4RescueRows: result.lostA4RescueRows,
      images: result.images.map((image: any) => ({
        ...
        dBaseline: image.dBaseline,
        missClassificationCounts:
          image.missClassificationCounts,
        missRows: image.missRows,
        a4LineNewRescueRows:
          image.a4LineNewRescueRows,
        lostA4RescueRows:
    ...
    <main style={styles.page}>
      <section style={styles.card}>
        <h1 style={styles.title}>
          Stage A6 Row Miss-Mechanism Diagnostic
        </h1>
        <p style={styles.text}>
          A5-Dを固定baselineとして再実行し、
          DでmissしたGT rowだけをpost-hocでA〜F分類します。
          detector、threshold、paper bbox、OCR engine、
          segmentation条件、candidate選択は変更しません。
        </p>
        <p style={styles.text}>
          A: rules/TSV upstream miss
          <br />
          B: raw TSV lineはGT rowをcoverするがconsolidationでloss
          <br />
          C: consolidated TSVはcoverするがrules-gap selectiveで除外
          <br />
          D: candidate bandはGT rowにoverlapするがcenter localizationが外れる
          <br />
          E: pathological TSV segmentation
          <br />
          F: unclassified
        </p>
      </section>
