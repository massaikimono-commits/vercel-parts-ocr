...
const EVAL_BRANCH =
  "eval/parts-ocr-stage-a12-phase-lattice";
const EVAL_HEAD =
  process.env.NEXT_PUBLIC_EVAL_HEAD || "unknown-preview-head";
const REVISION =
  "stage-a12-alt-self-seeded-phase-lattice-v1";
...
function buildManagementShortSummary(source: any) {
  ...
  const importantImages = (source?.images || [])
    ...
    .map((image: any) => ({
      ...
      P_SELF: image.variants?.P_SELF_phaseLockedSelfSeed?.gtCoverage,
      selectedPitchP: image.phaseTrace?.selectedPitch,
      selectedPitchSelf: image.phaseSelfTrace?.selectedPitch,
      phaseConsensusSupportSelf: image.phaseSelfTrace?.phaseConsensusSupport,
      selfSeededUsed:
        (image.phaseSelfTrace?.selfSeededLatticeCount || 0) > 0,
      DanchorCount: image.phaseSelfTrace?.dAnchorCount,
  ...
  const summary = {
    ...
    a11LReference: source?.a11LReference,
...
export default function StageA12Page() {
  ...
  async function run(files: FileList | null) {
    ...
    try {
      ...
      for (let i = 0; i < EXPECTED_FILES.length; i += 1) {
        ...
        setStatus(`${i + 1}/12 ${fileName} Stage A12解析中…`);
      ...
      const aggregate = {
        P_phaseLocked: summarizeVariant(
          images,
          "P_phaseLocked",
        ),
        P_SELF_phaseLockedSelfSeed: summarizeVariant(
          images,
          "P_SELF_phaseLockedSelfSeed",
        ),
      };

      const mechanismAggregate = {
        P_phaseLocked:
          aggregate.P_phaseLocked.mechanismRescue,
        P_SELF_phaseLockedSelfSeed:
          aggregate.P_SELF_phaseLockedSelfSeed.mechanismRescue,
      };

      const payload = {
        schema:
          "icb.parts-ocr.stage-a12-alt-self-seeded-phase-lattice.v1",
        revision: REVISION,
        ...
        experimentOnly: true,
        gtUsage:
          "post-hoc scoring/mechanism attribution only; no GT pitch hypothesis, phase selection, self-seed, candidate assignment, ranking, acceptance, stop, or control",
        baseline: {
          ...baseline,
      ...
      setResult(payload);
      setStatus(
        "12/12 Stage A12解析完了。総合管理用短縮summaryをコピーできます。",
      );
    } catch (error) {
      setStatus(
    ...
    <main style={styles.page}>
      <section style={styles.card}>
        <h1 style={styles.title}>
          Stage A12 ALT Self-Seeded Phase-Locked Row Lattice
        </h1>
        <p style={styles.text}>
          A5-D / A9 ALT / A10 G / A11 Lを同runで再現し、
          P（phase-locked / driftなし）と
          P_SELF（ALT self-seeded periodic consensus）を比較します。
          1 position欠落でgrowth停止せず、phase + k×pitchを固定評価します。
        </p>
      </section>
