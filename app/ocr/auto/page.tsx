...
import { prepareOCRInputFile, saveOCRTransferImage } from "../transfer";
...
function classify(text: string): { mode: Mode; reason: string; auto: boolean } {
  ...
  const supplierSignals = ["大一用品商会", "大一用品", "用品商会", "DAIICHI", "MC-E133", "MCE133", "07009330"];
  const dedicatedHits = supplierSignals.filter((x) => t.includes(normalize(x)));
  if (dedicatedHits.length) return { mode: "dedicated", reason: `大一用品商会の特徴を検出: ${dedicatedHits.join(" / ")}`, auto: true };
...
async function readDedicatedMarkers(worker: any, tesseract: any, file: File) {
  ...
  const crops = [relativeBox(paper, 0.00, 0.00, 1.00, 0.34), relativeBox(paper, 0.00, 0.32, 0.84, 0.28)];
  const texts: string[] = [];
  await worker.setParameters({ tessedit_pageseg_mode: tesseract.PSM?.SPARSE_TEXT ?? "11", preserve_interword_spaces: "1", tessedit_char_whitelist: "", user_defined_dpi: "300" });
...
export default function AutoOCRPage() {
  ...
  async function detect(file: File) {
    setBusy(true); setProgress(1); setMode(""); setReason(""); setRawText(""); setMessage("用紙の種類を判定しています…");
    let worker: any = null;
    try {
      const preparedFile = await prepareOCRInputFile(file);
      if (preview) URL.revokeObjectURL(preview);
      setPreview(URL.createObjectURL(preparedFile));
      await saveOCRTransferImage(preparedFile);
      const image = await prepareForDetection(preparedFile);
      const tesseract: any = await import("tesseract.js");
      worker = await tesseract.createWorker("jpn+eng", 1, { logger: (m: any) => { if (m.status === "recognizing text") setProgress(Math.max(1, Math.min(94, Math.round((m.progress || 0) * 80) + 5))); } });
      await worker.setParameters({ tessedit_pageseg_mode: tesseract.PSM?.SPARSE_TEXT ?? "11", preserve_interword_spaces: "1", tessedit_char_whitelist: "", user_defined_dpi: "300" });
      const result = await worker.recognize(image);
      let text = result.data.text || "";
      let judged = classify(text);
      if (judged.mode !== "dedicated") {
        setMessage("専用伝票の特徴を拡大して再確認しています…"); setProgress(82);
        const markerText = await readDedicatedMarkers(worker, tesseract, preparedFile);
        text = `${text}\n\n【専用判定用拡大OCR】\n${markerText}`;
      ...
      setRawText(text); setMode(judged.mode); setReason(judged.reason); setProgress(100);
      if (judged.mode === "dedicated") {
      ...
      } else setMessage("自動判定を確定できませんでした。下のボタンから専用OCRか汎用OCRを選んでください。");
    } catch (error) {
      console.error(error); setMode("unknown"); setReason("自動判定処理でエラーが出ました。誤ったOCRへ送らないよう自動移動を止めました。"); setMessage("自動判定を止めました。下から読み取り方式を選べます。");
    } finally {
      if (worker) await worker.terminate().catch(() => {}); setBusy(false);
    }
  }
  ...
  return (
    <main style={styles.page}>
      <section style={styles.card}>
        ...
        <p style={styles.text}>1回だけ撮影・選択すれば、共通画像前処理の後に用紙判定し、専用/汎用OCRへ同じ画像を引き継ぎます。</p>
      ...
      {mode && <section style={styles.card}>
        <h2 style={{ marginTop: 0 }}>判定結果</h2>
        <div style={mode === "unknown" ? styles.warning : { ...styles.notice, fontWeight: 800, fontSize: 18 }}>{mode === "dedicated" ? "大一用品商会 専用OCR" : mode === "general" ? "汎用A4・他社伝票OCR" : "判定保留"}</div>
        <p style={styles.text}>{reason}</p>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}><button style={styles.secondary} onClick={() => location.assign("/ocr")}>大一用品商会 専用OCR</button><button style={styles.secondary} onClick={() => location.assign("/ocr/general")}>汎用A4・他社伝票OCR</button></div>
      </section>}
      <section style={styles.card}><details><summary style={{ fontWeight: 700, cursor: "pointer" }}>判定用OCR詳細</summary><p style={styles.text}>共通前処理後の全体OCRに加えて、専用伝票の重要部分を拡大して再確認した文字も表示します。</p><textarea readOnly value={rawText} style={{ width: "100%", minHeight: 260, border: "1px solid #d6deea", borderRadius: 12, padding: 12, fontSize: 13, background: "#f8fafc" }} /></details></section>
    </main>
  );
}
