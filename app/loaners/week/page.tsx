...
export default function WeeklyLoanerPage() {
  ...
  async function loadWeek() {
    ...
    setRows(next);
    setMessage(next.some((x) => x.error) ? "一部の日で代車状況を取得できませんでした。" : "1週間の代車空きを表示しています。");
    setBusy(false);
  }
  ...
  return (
    <main className="loanerWeek">
      ...
      <section className="weekGrid">
        {rows.map((row) => {
          const low = row.available <= 1 && !row.error;
          return (
            <button
              key={row.day}
              className={`dayCard${row.error ? " error" : low ? " low" : ""}`}
      ...
      <style jsx global>{`
        *{box-sizing:border-box}body{margin:0;background:#f3f6fb;color:#172033;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}button,input{font:inherit}
        ...
        .controls{display:flex;gap:8px;flex-wrap:wrap;background:#fff;border:1px solid #d9e0ea;border-radius:16px;padding:12px}.controls input{border:1px solid #cbd6e3;border-radius:10px;padding:9px;background:#fff}.message{color:#667487;margin:10px 2px}.weekGrid{display:grid;grid-template-columns:repeat(7,minmax(0,1fr));gap:8px}.dayCard{display:block;text-align:left;color:#172033;padding:12px;min-height:190px}.dayCard.low{box-shadow:inset 0 4px 0 #d79a3d}.dayCard.error{box-shadow:inset 0 4px 0 #c84a4a}.dayHead{display:grid;border-bottom:1px solid #e4e9f0;padding-bottom:7px;margin-bottom:10px}.dayHead small{color:#7b8797;font-weight:600}.available{display:flex;align-items:baseline;gap:5px}.available span{font-size:11px;color:#657387}.available strong{font-size:32px;color:#25703c}.available small{color:#657387}.breakdown{display:grid;gap:4px;margin-top:8px;font-size:11px}.breakdown span{background:#f3f6fa;border-radius:7px;padding:5px}.reservationCount{font-size:11px;margin-top:8px;color:#586678}.warning{font-size:11px;margin-top:6px;background:#fff0db;color:#925b08;border-radius:7px;padding:5px}.errorText{color:#b33636}.footerActions{display:flex;gap:8px;justify-content:flex-end;margin-top:14px}
        @media(max-width:980px){.weekGrid{grid-template-columns:repeat(2,minmax(0,1fr))}.dayCard{min-height:150px}}@media(max-width:560px){.weekGrid{grid-template-columns:1fr}.dayCard{min-height:0}.controls{display:grid;grid-template-columns:1fr 1fr}.controls input{grid-column:1/-1}.footerActions{display:grid}.top>div span{display:none}}
      `}</style>
    </main>
  );
}
