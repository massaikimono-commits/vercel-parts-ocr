...
export default function CertificateTemplateRowFix(){
  useEffect(()=>{
    ...
    const run=async(srcKey:string)=>{const img=document.querySelector("img.preview") as HTMLImageElement|null;if(!img?.src||img.src!==srcKey)return;const v:Values={};try{const src=await sourceCanvas(img),p=paperBox(src),t:any=await import("./lib/tesseract-local");worker=await t.createWorker("jpn+eng",1);const one=t.PSM?.SINGLE_LINE??"7",sparse=t.PSM?.SPARSE_TEXT??"11",block=t.PSM?.SINGLE_BLOCK??"6";
