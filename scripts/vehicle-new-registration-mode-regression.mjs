...
if (!/const\\s+newRegistrationMode\\s*=\\s*searchParams\\.get\\(\\s*["\']mode["\']\\s*\\)\\s*===\\s*["\']new["\']\\s*;/.test(v2)) throw new Error("v2 mode=new semantic binding missing");
if (!/<VehicleWorkflowFast\\s+newRegistrationMode=\\{newRegistrationMode\\}\\s*\\/>/.test(v2)) throw new Error("v2 new-registration mode forwarding missing");
