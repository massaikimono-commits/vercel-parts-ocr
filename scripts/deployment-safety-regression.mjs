...
if (!fs.existsSync(netlifyPath)) {
  ...
  for (const requiredLine of requiredNetlifyLines) {
    if (!netlify.includes(requiredLine)) fail(`Netlify deployment lock is missing or changed: ${requiredLine}`);
  }
...
if (!fs.existsSync(vercelPath)) {
  ...
  if (vercel) {
    if (vercel?.git?.deploymentEnabled !== false) fail("Vercel Git auto-deploy must stay disabled (git.deploymentEnabled=false).");
    if (Object.prototype.hasOwnProperty.call(vercel, "ignoreCommand")) fail("Legacy Vercel ignoreCommand/[deploy] gate must stay retired. deploymentEnabled=false is the single Git auto-deploy lock.");
  }
}
...
if (fs.existsSync(workflowDir)) {
  ...
  for (const file of fs.readdirSync(workflowDir)) {
    ...
    for (const pattern of riskyPatterns) if (pattern.test(body)) fail(`Potential direct deployment command found in ${fullPath}: ${pattern}`);
  }
}
