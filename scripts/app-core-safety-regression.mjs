...
const dailyReportTemplate = read("app","schedule","daily-report-template.ts");
...
assert(vehicleBulk.includes("stagedImportId") && vehicleBulk.includes('status:"NEEDS_REVIEW"'), "bulk PDF matching failures must keep staged imports visible as review items instead of orphaning them");
assert(vehicleBulk.includes("if(updateError) throw updateError"), "bulk PDF staging status updates must not silently fail");
...
assert(reportPrint.includes(".deliveryEntry .entryName{left:.8%;width:31.6%") && reportPrint.includes(".inboundEntry .entryName{left:.8%;width:33.4%"), "daily report customer names must align to the upper user-name cells on both A3 halves");
assert(reportPrint.includes(".deliveryEntry .entryReason{left:33.3%;width:19.9%") && reportPrint.includes(".inboundEntry .entryReason{left:34.4%;width:20.6%"), "daily report intake reason must align to the existing PDF parenthesis cell below 車両No.");
assert(reportPrint.includes("plannedDeliveryItem") && reportPrint.includes('className="plannedCustomer"') && reportPrint.includes('className="plannedVehicle"') && reportPrint.includes('deadlineStack(deadline, "plannedDeadline")'), "planned-delivery section must render source-PDF columns separately");
assert(reportPrint.includes("grid-template-columns:35.5% 35.8% 28.7%"), "planned-delivery columns must match the supplied A3 PDF user / vehicle(work) / deadline widths");
...
assert(dailyReportTemplate.includes("842 x 1191 pt"), "daily report template must document the exact supplied PDF page geometry");
assert(dailyReportTemplate.includes("stayingVehicles: { x: 0.0748, y: 0.8208") && dailyReportTemplate.includes("bodyShopVehicles: { x: 0.4087, y: 0.8208"), "bottom daily-report sections must start after the source PDF vertical labels");
assert(dailyReportTemplate.includes("plannedDeliveries: { x: 0.7101, y: 0.7395"), "planned-delivery data must start below the source PDF header and right of its vertical label");
