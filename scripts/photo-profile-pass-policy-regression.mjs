...
function accepted(values, key) {
  ...
  return unique.length === 1 && (categorical || observations.length >= 2) ? unique[0] : "";
}
...
assert.equal(accepted(["5", ""], "seatingCapacity"), "", "one successful numeric OCR pass plus one empty pass must not count as two-pass agreement");
...
assert.ok(
  src.includes('if (!value) continue;'),
  "runtime profile observations must continue dropping empty OCR passes before two-pass confidence is counted"
);
assert.ok(
