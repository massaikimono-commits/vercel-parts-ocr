...
function runScenario(name, targets) {
  ...
  // The detector intentionally returns horizontal crop centers only. Painting
  // progressively different Y positions verifies slanted/perspective layouts
  // remain detectable without inventing a Y-coordinate production contract.
  for (const target of targets) {
    if (!actual.some((item) => Math.abs(item.x - target.x) <= 0.025)) {
      throw new Error(`${name}: missed QR center x=${target.x} (painted y=${target.y}); actual=${actual.map((item) => item.x).join(",")}`);
    }
  }
  if (averageMs > 750) {
  ...
  console.log(`PASS ${name}: candidates=${actual.length}; average=${averageMs.toFixed(1)}ms`);
}

runScenario("QR perspective rising", baseTargets);
runScenario(
  "QR perspective falling",
  baseTargets.map((target, index) => ({
    ...target,
    y: 0.923 - index * 0.017,
  })),
);
